...
const analyticsLabGlobal = globalThis as typeof globalThis & {
  ...
  __traderIntelligenceAnalyticsLabRuntimeBuilds?: Map<
    string,
    Promise<AnalyticsLabRuntime>
  >;
};
...
function authorityBindingIdentity(binding: AuthorityBinding): {
  canonicalOwnerKey: string;
  canonicalAccountKey: string;
} {
  ...
  return { canonicalOwnerKey, canonicalAccountKey };
}
...
function developmentAuthority(
  ...
  const { canonicalOwnerKey, canonicalAccountKey } =
    authorityBindingIdentity(binding);
  ...
  const startingInventories =
    binding.attachment.startingInventories.map((inventory) => {
      ...
      const stableInstrumentKey = execution.content.stableInstrumentKey;
      if (stableInstrumentKey === null) {
        throw new Error("Analytics Lab canonical execution is invalid.");
      }
      const rebuilt = buildStartingInventoryContract({
        state: inventory.state === "unknown" ? "unknown" : "proven_flat",
        asOf: execution.content.executedAt,
        coverageState:
          inventory.state === "unknown" ? "unknown" : "complete",
        ledgerIdentity: {
...
function buildPersistedAnalyticsLabRuntime(args: {
  authoritySource: ReadOnlySnapshotAuthoritySource;
  authorityDirectory: string;
  canonicalOwnerKey: string;
  canonicalAccountKey: string;
  cacheKey: string;
}): AnalyticsLabRuntime {
  const exactAuthority = args.authoritySource.readExactAuthority();
  if (exactAuthority.state === "unavailable") {
    throw new Error(exactAuthority.reasonCode);
  }
  const snapshotSource = createSnapshotTradeQueryDatasetSource(
    args.authoritySource,
  );
  const derived = snapshotSource.readVerifiedDataset();
  if (!derived.ok) {
    throw new Error(`${derived.error.code}:${derived.error.path}`);
  }
  const source = createInMemoryVerifiedTradeQueryDatasetSource(derived.value);
  const currencies = [
  ...
  const runtime: AnalyticsLabRuntime = Object.freeze({
    ...
    authorityDirectory: args.authorityDirectory,
    minimumDate: dates[0],
...
async function resolvePersistedAnalyticsLabRuntime(): Promise<AnalyticsLabRuntime> {
  ...
  let createAuthoritySource: () => ReadOnlySnapshotAuthoritySource;
  ...
  if (deployment.ok) {
    ...
    if (useDevelopmentBinding) {
      const development = authorityBindingIdentity(binding);
      if (
        development.canonicalOwnerKey !== canonicalOwnerKey ||
        development.canonicalAccountKey !== canonicalAccountKey
      ) {
        throw new Error("Analytics Lab development scope is invalid.");
      }
      createAuthoritySource = () => developmentAuthority(binding).source;
    } else {
      createAuthoritySource = () => {
        ...
        return configuredAuthority.value;
      };
    }
  } else {
    if (process.env.NODE_ENV === "production") {
  ...
  const activeBuilds =
    analyticsLabGlobal.__traderIntelligenceAnalyticsLabRuntimeBuilds ??
    new Map<string, Promise<AnalyticsLabRuntime>>();
  analyticsLabGlobal.__traderIntelligenceAnalyticsLabRuntimeBuilds =
    activeBuilds;
  const existingBuild = activeBuilds.get(cacheKey);
  if (existingBuild) return existingBuild;

  const pendingBuild = Promise.resolve().then(() =>
    buildPersistedAnalyticsLabRuntime({
  ...
  activeBuilds.set(cacheKey, pendingBuild);
  try {
    return await pendingBuild;
  } finally {
    if (activeBuilds.get(cacheKey) === pendingBuild) {
      activeBuilds.delete(cacheKey);
    }
  }
}
