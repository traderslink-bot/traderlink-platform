  ...
  return <Stack spacing={1.25}>
    <HorizontalScrollRegion label="Scaling out trades" minTableWidth={1780} stickyFirstColumn>
      <Table size="small"><TableHead><TableRow>
        ...
        <TableCell><Button endIcon={<OpenInNewIcon fontSize="small" />} href={offline ? `/trade-tracker/${row.trackerDate}` : `/trade-tracker/${row.trackerDate}?${new URLSearchParams({ interval: "1m", trade: row.roundTripId, basis: moneyBasis }).toString()}`} size="small" variant="outlined">Full analysis</Button></TableCell>
      </TableRow>;
      })}</TableBody></Table>
    </HorizontalScrollRegion>
  ...
  return <Stack spacing={1.25}>
    ...
    <HorizontalScrollRegion label="Saved event price paths" minTableWidth={1540} stickyFirstColumn>
      <Table size="small"><TableHead><TableRow>
        ...
        })}<TableCell><Button endIcon={<OpenInNewIcon fontSize="small" />} href={offline ? `/trade-tracker/${event.trackerDate}` : `/trade-tracker/${event.trackerDate}?${new URLSearchParams({ interval: "1m", trade: event.roundTripId, basis: model.moneyBasis }).toString()}`} size="small" variant="outlined">Full analysis</Button></TableCell></TableRow>)}</TableBody>
      </Table>
    </HorizontalScrollRegion>
    {paginationAtBottom ? <TradeAnalyzerTablePagination onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1); }} page={currentPage} pageSize={pageSize} rowCount={grouped.length} /> : null}
    </>}
