    ...
    <Stack spacing={2.5}>
      ...
      <Box sx={{ display: "grid", gap: 1.5, gridTemplateColumns: { xs: "repeat(2, minmax(0, 1fr))", md: "repeat(3, minmax(0, 1fr))" } }}>
        {ANALYTICS_OVERVIEW_METRICS.map((definition) => {
          ...
          return <DashboardMetricCard caption={definition.caption} key={definition.id} label={definition.label} value={metric ? formatJournalAnalyticsMetric(metric) : "Unavailable"} />;
        })}
      </Box>
