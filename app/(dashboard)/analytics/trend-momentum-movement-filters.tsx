...
export function MovementIndicatorFilters({ queryString, onChange, coverage, mode = "movement" }: {
  ...
  return <Stack spacing={1}>
    <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
      <TextField select size="small" label="Indicator timeframe" value={filters.interval} onChange={(e) => change("interval", e.target.value)} helperText={mode === "patterns" ? "Saved indicator context before the execution; pattern timeframe stays unchanged." : "Saved context before the execution; movement still uses one-minute candles."}><MenuItem value="1m">1 minute</MenuItem><MenuItem value="5m">5 minutes</MenuItem></TextField>
    ...
    <Typography variant="body2" color="text.secondary">{coverage.matched} matching {mode === "patterns" ? "pattern occurrences" : "executions"} · {coverage.notMatched} outside these conditions · {coverage.unavailable} without the required indicator context. {mode === "patterns" ? "These filters apply to pattern summaries and their evidence rows." : "These filters apply to the movement cards and both tables."}</Typography>
  </Stack>;
}
