...
export function GreenToRedAnalysis({
  ...
  return <Stack spacing={1.75}>
    <Box sx={{ display: "grid", gap: 1, gridTemplateColumns: { xs: "minmax(0, 1fr)", sm: "repeat(2, minmax(0, 1fr))", lg: "repeat(5, minmax(0, 1fr))" } }}>
      ...
      <Metric detail={`${endedRedRows.length} of ${rows.length} · ${money(sum(endedRedRows, (row) => row.finalGrossPnlDecimal), currency)}`} help="Trades that reached +20% or more and completed with a negative Gross P/L." label="Finished red" tone={financialOutcomeColor(sum(endedRedRows, (row) => row.finalGrossPnlDecimal))} value={percent(rate(endedRedRows.length, rows.length))} />
    </Box>
      ...
      {filteredRows.length === 0 ? <Typography color="text.secondary">No trades match this filter.</Typography> : <Stack spacing={1}>
        ...
        <HorizontalScrollRegion label="Green-to-red trade records" minTableWidth={1540} stickyFirstColumn>
          <Table size="small"><TableHead><TableRow>
            ...
            <TableCell>{row.firstRedAfterTwentyAtUtcSeconds === null ? <Typography color="text.secondary" variant="body2">Did not turn red</Typography> : <><Typography component="div" sx={{ fontWeight: 750 }} variant="body2">{clockTime(row.firstRedAfterTwentyAtUtcSeconds, timezone)}</Typography><Typography color="text.secondary" component="div" variant="caption">{elapsedMinutes(row.firstReachedTwentyAtUtcSeconds, row.firstRedAfterTwentyAtUtcSeconds)} after +20%</Typography>{row.firstRecoveryAfterRedAtUtcSeconds !== null ? <Typography color="text.secondary" component="div" variant="caption">Recovered {clockTime(row.firstRecoveryAfterRedAtUtcSeconds, timezone)}</Typography> : <Typography color="text.secondary" component="div" variant="caption">No later recovery measured</Typography>}</>}</TableCell>
