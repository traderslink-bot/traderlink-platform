...
export default async function CoachReviewWorkspacePage({params}:{params:Promise<{communitySlug:string;relationshipId:string;reviewId:string}>}){
 ...
 return <DashboardPage>
  ...
  <Stack direction={{xs:"column",sm:"row"}} spacing={1} sx={{alignItems:{sm:"center"},justifyContent:"space-between"}}><Box><Typography component="h1" variant="h1">{review.title}</Typography><Typography color="text.secondary">{relationship.studentDisplayName} · {relationship.planName}</Typography></Box><Button href={`/communities/${communitySlug}/workspace/students/${relationshipId}`} variant="outlined">Student workspace</Button></Stack>
  <Grid container spacing={2}>
   <Grid size={{xs:12,lg:8}}><DashboardPanel title="Review"><CoachReviewEditor communitySlug={communitySlug} isReview={isReview} previousFocus={previous?.nextFocus??null} relationshipId={relationshipId} review={review}/></DashboardPanel></Grid>
   <Grid size={{xs:12,lg:4}}><Stack spacing={2}><DashboardPanel title="Review agreement"><Stack spacing={1}><Typography fontWeight={800}>{relationship.planName}</Typography><Typography variant="body2">{review.reviewType.replaceAll("_"," ")}</Typography>{review.periodStart?<Typography variant="body2">{review.periodStart} – {review.periodEnd}</Typography>:null}<Chip label={`${trades.length} ${trades.length===1?"trade":"trades"}`} sx={{alignSelf:"flex-start"}}/></Stack></DashboardPanel><DashboardPanel title="Trades to review"><Stack spacing={1}>{trades.map((trade,index)=><Stack direction="row" key={trade} sx={{alignItems:"center",justifyContent:"space-between"}}><Typography fontWeight={800}>Trade {index+1}</Typography><Button size="small">Review</Button></Stack>)}{!trades.length?<Typography color="text.secondary" variant="body2">No trades selected</Typography>:null}</Stack></DashboardPanel></Stack></Grid>
  </Grid>
 </DashboardPage>;
}
