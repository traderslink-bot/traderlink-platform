...
export function CoachTradeReviewWorkspace({initialTradeIds,isReview}:{initialTradeIds:readonly string[];isReview:boolean}){
 ...
 return <Stack spacing={2}>
  <DashboardPanel title="Trades"><Stack spacing={2}>
   <ToggleButtonGroup exclusive fullWidth onChange={(_,next)=>{if(next){setView(next);setChecks([]);}}} size="small" value={view}><ToggleButton value="find">Find trades</ToggleButton><ToggleButton title="Trades to review" value="queue">To review {queued.size-saved.size}</ToggleButton><ToggleButton title="Saved trade reviews" value="saved">Saved {saved.size}</ToggleButton></ToggleButtonGroup>
...
function drawerTitle(value:typeof null|"details"|"chart"|"trades"|"analytics"|"notes"|"history"|"rules"){return ({details:"Trade Details",chart:"Stock chart",trades:"All shared trades",analytics:"Analytics",notes:"Journal notes",history:"Coaching history",rules:"Rules"} as const)[value??"details"]}
