...
function tradeMoment(date: string | null, time: string | null, price: string | null, currency: string): string {
  const day = date ? new Intl.DateTimeFormat("en-US", { day: "numeric", month: "short" }).format(new Date(`${date}T12:00:00`)) : "—";
  const amount = price ? formatJournalAnalyticsMoney(price, currency) : "—";
  return `${day} ${time ?? "—"} · ${amount}`;
}
...
function SavedTradePanel({ accountCurrency, accountTimezone, currentAccountStillMatches, expectedAccountSelectionRef, offlineScopeRef, onClose, row, startingTab }: Readonly<{ accountCurrency: string; accountTimezone: string; currentAccountStillMatches: () => Promise<boolean>; expectedAccountSelectionRef: string; offlineScopeRef: string; onClose: () => void; row: WorkspaceTradeLibraryRow | null; startingTab: number }>) {
  ...
  if (!row) return null;
  const targets = row.exitDate === null ? [] : [{ closeLocalDate: row.exitDate, closedAtUtc: `${row.exitDate}T00:00:00.000Z`, direction: row.direction, displayedSymbol: row.symbol, roundTripId: row.roundTripId }];
...
export function WorkspaceTradeLibrary({ accountCurrency, accountTimezone, addTradeOpen, expectedAccountSelectionRef, offlineScopeRef, onAddTradeClose, trades }: Readonly<{ accountCurrency: string; accountTimezone: string; addTradeOpen: boolean; expectedAccountSelectionRef: string; offlineScopeRef: string; onAddTradeClose: () => void; trades: WorkspaceTradeLibraryModel }>) {
  const router = useRouter(); const [model, setModel] = useState(trades); const [detail, setDetail] = useState<WorkspaceTradeLibraryRow | null>(null); const [detailTab, setDetailTab] = useState(0); const [ticker, setTicker] = useState(trades.query.searchTicker); const [filter, setFilter] = useState<TradeStateFilter>(trades.query.filter); const [startDate, setStartDate] = useState(trades.query.startDate ?? ""); const [endDate, setEndDate] = useState(trades.query.endDate ?? ""); const [sort, setSort] = useState<TradeSort>(trades.query.sort); const [group, setGroup] = useState<TradeGroup>(trades.query.group); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null); const [tableDelete, setTableDelete] = useState<Readonly<{ deleteRef: string; symbol: string }> | null>(null); const [deletingTableExecution, setDeletingTableExecution] = useState(false); const [tableDeleteError, setTableDeleteError] = useState<string | null>(null);
  ...
  const actionButtons = (row: WorkspaceTradeLibraryRow) => { const deletable = row.editableExecutions.filter((execution) => execution.deleteRef !== null); const deletion = deletable.length === 1 ? deletable[0]! : null; return <Stack direction="row" spacing={0.25}>{row.exitDate !== null ? <ActionButton label="Review trade" onClick={() => { setDetail(row); setDetailTab(1); }}><FactCheckOutlinedIcon fontSize="small" /></ActionButton> : null}<ActionButton label="Edit trade" onClick={() => { setDetail(row); setDetailTab(0); }}><EditRoundedIcon fontSize="small" /></ActionButton>{deletion?.deleteRef ? <ActionButton desktopOnly label="Delete execution" onClick={() => void prepareTableDelete(row, deletion.deleteRef)}><DeleteOutlineRoundedIcon fontSize="small" /></ActionButton> : null}<ActionButton label="Open Trade Analyzer" onClick={() => { setDetail(row); setDetailTab(2); }}><InsightsRoundedIcon fontSize="small" /></ActionButton></Stack>; };
  ...
  const desktopColumns = "minmax(94px, .8fr) minmax(62px, .65fr) minmax(52px, .55fr) minmax(76px, .7fr) minmax(52px, .55fr) minmax(142px, 1.15fr) minmax(142px, 1.15fr) minmax(64px, .6fr) minmax(86px, .8fr) 132px";
  return <>{error ? <Typography color="error.main" variant="body2">{error}</Typography> : null}<Box sx={{ border: 1, borderColor: "divider", borderRadius: 2, mt: 1, overflow: "hidden" }}><Box sx={{ display: { xs: "none", md: "grid" }, gridTemplateColumns: desktopColumns, gap: 1, px: 1, py: 0.75, bgcolor: "action.hover" }}>{["Date", "Ticker", "Side", "State", "Shares", "Entry", "Exit", "Executions", "P/L", "Actions"].map((label) => <Typography key={label} variant="caption">{label}</Typography>)}</Box>{model.projectionState !== "ready" || model.rows.length === 0 ? <Typography color="text.secondary" sx={{ p: 2 }} variant="body2">{emptyState}</Typography> : grouped.map(({ row, heading }) => <Box key={row.roundTripId}>{heading ? <Typography sx={{ bgcolor: "action.hover", fontWeight: 800, px: 1.5, py: 0.5 }} variant="caption">{heading}</Typography> : null}<Box sx={{ borderTop: 1, borderColor: "divider" }}><Box sx={{ alignItems: "center", display: { xs: "none", md: "grid" }, gap: 1, gridTemplateColumns: desktopColumns, minHeight: 46, px: 1, py: 0.25 }}><Typography variant="body2">{shortDate(row.date)}</Typography><Typography sx={{ fontWeight: 800 }} variant="body2">{row.symbol}</Typography><Typography variant="body2">{row.direction === "long" ? "Long" : "Short"}</Typography><Typography variant="body2">{row.status}</Typography><Typography variant="body2">{formatJournalAnalyticsDecimal(row.buyQuantityDecimal)}</Typography><Typography noWrap title={tradeMoment(row.entryDate, row.entryTime, row.entryPriceDecimal, row.tradeCurrency)} variant="caption">{tradeMoment(row.entryDate, row.entryTime, row.entryPriceDecimal, row.tradeCurrency)}</Typography><Typography noWrap title={tradeMoment(row.exitDate, row.exitTime, row.exitPriceDecimal, row.tradeCurrency)} variant="caption">{tradeMoment(row.exitDate, row.exitTime, row.exitPriceDecimal, row.tradeCurrency)}</Typography><Typography variant="body2">{row.executionCount}</Typography><Typography color={financialOutcomeColor(row.gainLossDecimal)} sx={{ fontWeight: 800 }} variant="body2">{tradeMoney(row)}</Typography>{actionButtons(row)}</Box><Box sx={{ display: { md: "none" }, p: 1.25 }}><Stack direction="row" sx={{ alignItems: "flex-start", justifyContent: "space-between" }}><Box><Typography sx={{ fontWeight: 800 }}>{row.symbol}</Typography><Typography variant="caption">{shortDate(row.date)} · {row.direction === "long" ? "Long" : "Short"} · {row.status}</Typography></Box><Typography color={financialOutcomeColor(row.gainLossDecimal)} sx={{ fontWeight: 800 }}>{tradeMoney(row)}</Typography></Stack><Typography display="block" variant="caption">{formatJournalAnalyticsDecimal(row.buyQuantityDecimal)} shares · {row.executionCount} execution{row.executionCount === 1 ? "" : "s"}</Typography><Typography display="block" noWrap title={tradeMoment(row.entryDate, row.entryTime, row.entryPriceDecimal, row.tradeCurrency)} variant="caption">Entry {tradeMoment(row.entryDate, row.entryTime, row.entryPriceDecimal, row.tradeCurrency)}</Typography><Typography display="block" noWrap title={tradeMoment(row.exitDate, row.exitTime, row.exitPriceDecimal, row.tradeCurrency)} variant="caption">Exit {tradeMoment(row.exitDate, row.exitTime, row.exitPriceDecimal, row.tradeCurrency)}</Typography>{actionButtons(row)}</Box></Box></Box>)}</Box>{model.continuationCursor ? <Stack sx={{ alignItems: "center", mt: 1 }}><Button disabled={loading} onClick={() => void load(model.continuationCursor, true)} variant="outlined">{loading ? "Loading…" : "Load more"}</Button></Stack> : null}<WorkspaceTradeDrawer accountCurrency={accountCurrency} accountTimezone={accountTimezone} addOpen={addTradeOpen} currentAccountStillMatches={currentAccountStillMatches} detail={detail} expectedAccountSelectionRef={expectedAccountSelectionRef} offlineScopeRef={offlineScopeRef} onClose={() => { if (addTradeOpen) { onAddTradeClose(); window.location.reload(); } else setDetail(null); }} startingTab={detailTab} /><Dialog fullWidth maxWidth="xs" onClose={() => { if (!deletingTableExecution) setTableDelete(null); }} open={tableDelete !== null}><DialogTitle>Delete this execution?</DialogTitle><DialogContent><Typography>This may change whether the position is open or closed.</Typography>{tableDeleteError ? <Alert severity="error" sx={{ mt: 1 }}>{tableDeleteError}</Alert> : null}</DialogContent><DialogActions><Button disabled={deletingTableExecution} onClick={() => setTableDelete(null)}>Cancel</Button><Button color="error" disabled={deletingTableExecution} onClick={() => void deleteFromTable()} variant="contained">{deletingTableExecution ? "Deleting…" : "Delete execution"}</Button></DialogActions></Dialog></>;
}
