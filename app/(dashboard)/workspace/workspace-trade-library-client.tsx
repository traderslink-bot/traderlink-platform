...
export function WorkspaceTradeLibrary({ accountCurrency, accountTimezone, addTradeOpen, customEndDate, customStartDate, expectedAccountSelectionRef, offlineScopeRef, onAddTradeClose, periodEndDate, periodStartDate, trades }: Readonly<{ accountCurrency: string; accountTimezone: string; addTradeOpen: boolean; customEndDate: string | null; customStartDate: string | null; expectedAccountSelectionRef: string; offlineScopeRef: string; onAddTradeClose: () => void; periodEndDate: string | null; periodStartDate: string | null; trades: WorkspaceTradeLibraryModel }>) {
  const router = useRouter(); const searchParams = useSearchParams(); const { openChart } = useDashboardChart(); const [model, setModel] = useState(trades); const [detailsTrade, setDetailsTrade] = useState<WorkspaceTradeLibraryRow | null>(null); const [detailsTab, setDetailsTab] = useState<"details" | "analyzer">("details"); const [atomicEditTrade, setAtomicEditTrade] = useState<WorkspaceTradeLibraryRow | null>(null); const [atomicEditTab, setAtomicEditTab] = useState<"trade" | "journal">("trade"); const [ticker, setTicker] = useState(trades.query.searchTicker); const [filter, setFilter] = useState<TradeStateFilter>(trades.query.filter); const [startDate, setStartDate] = useState(customStartDate ?? ""); const [endDate, setEndDate] = useState(customEndDate ?? ""); const [sort, setSort] = useState<TradeSort>(trades.query.sort); const [group, setGroup] = useState<TradeGroup>(trades.query.group); const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null); const [tableDelete, setTableDelete] = useState<Readonly<{ deleteRef: string; executionCount: number; roundTripId: string; symbol: string }> | null>(null); const [deletingTableExecution, setDeletingTableExecution] = useState(false); const [tableDeleteError, setTableDeleteError] = useState<string | null>(null);
  ...
  useEffect(() => { const handle = window.setTimeout(() => { void load(null, false); }, 80); return () => window.clearTimeout(handle); }, [ticker, filter, startDate, endDate, sort, group, periodEndDate, periodStartDate]);
  useEffect(() => { const handle = window.setTimeout(() => { const next = new URLSearchParams(searchParams.toString()); const values = { searchTicker: ticker.trim(), filter, startDate, endDate, sort, group }; for (const [key, value] of Object.entries(values)) { const isDefault = !value || (key === "filter" && value === "all") || (key === "sort" && value === "newest") || (key === "group" && value === "none"); if (isDefault) next.delete(key); else next.set(key, value); } if (next.toString() !== searchParams.toString()) router.replace(next.size ? `/workspace?${next.toString()}` : "/workspace", { scroll: false }); }, 700); return () => window.clearTimeout(handle); }, [endDate, filter, group, router, searchParams, sort, startDate, ticker]);
  ...
  const clearFilters = () => { setTicker(""); setFilter("all"); setStartDate(""); setEndDate(""); setSort("newest"); setGroup("none"); };
  ...
  return <>
    ...
    <Box sx={{ alignItems: "center", display: { xs: "none", md: "grid" }, gap: 1, gridTemplateColumns: "10ch 210px 18ch 18ch 25ch 12ch max-content", mt: 0.5, overflowX: "auto", pb: 0.25, pt: 1 }}>
      ...
      <TextField label="To" onChange={(event) => setEndDate(event.target.value)} size="small" slotProps={{ inputLabel: { shrink: true } }} type="date" value={endDate} />
      <TextField label="Sort" onChange={(event) => setSort(event.target.value as TradeSort)} select size="small" value={sort}>
    ...
    <Box sx={{ display: { md: "none" }, mt: 1 }}>
      <Button
        ...
        endIcon={mobileFiltersOpen ? <ExpandLessRoundedIcon /> : <ExpandMoreRoundedIcon />}
      ...
      <Collapse in={mobileFiltersOpen}>
        <Stack spacing={1.25} sx={{ border: 1, borderColor: "divider", borderRadius: 2, mt: 1, p: 1.5 }}>
          <TextField label="Ticker" onChange={(event) => setTicker(event.target.value.toUpperCase())} size="small" slotProps={{ htmlInput: { maxLength: 5 } }} value={ticker} />
          <TextField label="Filter" onChange={(event) => setFilter(event.target.value as TradeStateFilter)} select size="small" value={filter}><MenuItem value="all">All</MenuItem><MenuItem value="open">Open</MenuItem><MenuItem value="swing">Swing</MenuItem><MenuItem value="closed">Closed</MenuItem><MenuItem value="fees_not_entered">Fees not entered</MenuItem></TextField>
          <Stack direction="row" spacing={1}>
            <TextField fullWidth label="From" onChange={(event) => setStartDate(event.target.value)} size="small" slotProps={{ inputLabel: { shrink: true } }} type="date" value={startDate} />
            <TextField fullWidth label="To" onChange={(event) => setEndDate(event.target.value)} size="small" slotProps={{ inputLabel: { shrink: true } }} type="date" value={endDate} />
          </Stack>
          <TextField label="Sort" onChange={(event) => setSort(event.target.value as TradeSort)} select size="small" value={sort}>
            <MenuItem value="newest">Date: newest</MenuItem><MenuItem value="oldest">Date: oldest</MenuItem><MenuItem value="ticker_asc">Ticker: A-Z</MenuItem><MenuItem value="ticker_desc">Ticker: Z-A</MenuItem><MenuItem value="direction_asc">Side: A-Z</MenuItem><MenuItem value="direction_desc">Side: Z-A</MenuItem><MenuItem value="status_asc">Status: A-Z</MenuItem><MenuItem value="status_desc">Status: Z-A</MenuItem><MenuItem value="buy_quantity">Shares: high to low</MenuItem><MenuItem value="buy_quantity_asc">Shares: low to high</MenuItem><MenuItem value="position">POS: high to low</MenuItem><MenuItem value="position_asc">POS: low to high</MenuItem><MenuItem value="entry">Entry: high to low</MenuItem><MenuItem value="entry_asc">Entry: low to high</MenuItem><MenuItem value="exit">Exit: high to low</MenuItem><MenuItem value="exit_asc">Exit: low to high</MenuItem><MenuItem value="entry_value">Entry value: high to low</MenuItem><MenuItem value="entry_value_asc">Entry value: low to high</MenuItem><MenuItem value="hold">Hold: longest</MenuItem><MenuItem value="hold_asc">Hold: shortest</MenuItem><MenuItem value="pnl_high">Gain/Loss: high to low</MenuItem><MenuItem value="pnl_low">Gain/Loss: low to high</MenuItem>
          </TextField>
          ...
          <Stack direction="row" spacing={1}>
            ...
            <Button disabled={sort === "newest"} onClick={() => setSort("newest")} size="small">Reset</Button>
          </Stack>
        </Stack>
      </Collapse>
    </Box>
      ...
      {model.projectionState !== "ready" || model.rows.length === 0 ? <Typography color="text.secondary" sx={{ p: 2 }} variant="body2">{emptyState}</Typography> : grouped.map(({ row, heading }) => <Box key={row.roundTripId}>
        ...
        <Box sx={{ borderTop: 1, borderColor: "divider" }}>
          ...
          <Box sx={{ display: { md: "none" }, p: 1.25 }}>
            <Stack direction="row" sx={{ alignItems: "flex-start", justifyContent: "space-between" }}><Box><Typography sx={{ color: "warning.main", fontWeight: 800 }}>{row.symbol}</Typography><Typography variant="caption">{shortDate(row.date)} · {row.direction === "long" ? "Long" : "Short"} · {row.status}</Typography></Box><Typography sx={{ color: financialOutcomeColor(row.gainLossDecimal), fontWeight: 800 }}>{tradeMoney(row)}</Typography></Stack>
    ...
    <WorkspaceAtomicTradeEditDrawer expectedAccountSelectionRef={expectedAccountSelectionRef} journalTarget={atomicEditTrade ? { closeLocalDate: atomicEditTrade.exitDate, closedAtUtc: null, direction: atomicEditTrade.direction, displayedSymbol: atomicEditTrade.symbol, roundTripId: atomicEditTrade.roundTripId } : null} onClose={closeAtomicEdit} onSaved={() => { closeAtomicEdit(); window.location.reload(); }} open={atomicEditTrade !== null} roundTripId={atomicEditTrade?.roundTripId ?? null} startingTab={atomicEditTab} />
