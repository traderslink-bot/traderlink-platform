...
import { Alert, Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, Drawer, IconButton, MenuItem, Stack, Tab, TableSortLabel, Tabs, TextField, Tooltip, Typography } from "@mui/material";
...
import { WorkspaceTradeAnalyzerPanel } from "./workspace-trade-analyzer-panel";
...
function SavedTradePanel({ accountCurrency, accountTimezone, currentAccountStillMatches, expectedAccountSelectionRef, offlineScopeRef, onClose, onTabChange, row, tab }: Readonly<{ accountCurrency: string; accountTimezone: string; currentAccountStillMatches: () => Promise<boolean>; expectedAccountSelectionRef: string; offlineScopeRef: string; onClose: () => void; onTabChange: (nextTab: number) => void; row: WorkspaceTradeLibraryRow | null; tab: number }>) {
  const router = useRouter();
  const [addingExecution, setAddingExecution] = useState(false);
  if (!row) return null;
  ...
  return <Box sx={{ flex: 1, minHeight: 0, overflowY: "auto", p: tab === 1 ? 0 : 2 }}>
    {tab === 0 ? <Stack spacing={1}>
      <Typography color={financialOutcomeColor(row.gainLossDecimal)} variant="h5">{tradeMoney(row)}</Typography>
      {row.editableExecutions.map((execution) => <WorkspaceExecutionEditForm currentAccountStillMatches={currentAccountStillMatches} execution={execution} expectedAccountSelectionRef={expectedAccountSelectionRef} key={execution.editRef} onDeleted={onClose} symbol={row.symbol} />)}
      {addingExecution ? <AddTradePanel accountCurrency={accountCurrency} accountTimezone={accountTimezone} embedded expectedAccountSelectionRef={expectedAccountSelectionRef} fixedSymbol={row.symbol} initialWorkspaceStyle={row.tradeStyle === "swing" ? "swing" : "day_trade"} offlineScopeRef={offlineScopeRef} onClose={() => setAddingExecution(false)} onSaved={() => { router.refresh(); onClose(); }} /> : <Button onClick={() => setAddingExecution(true)} sx={{ alignSelf: "flex-start" }}>Add execution</Button>}
    </Stack> : null}
    {tab === 1 ? <TradeExplorerReviewEditor embedded expectedAccountSelectionRef={expectedAccountSelectionRef} onClose={() => onTabChange(0)} onSelectTrade={() => undefined} open selectedRoundTripId={row.roundTripId} showTagSelectionCount={false} showTradeNavigation={false} trades={targets} /> : null}
    {tab === 2 ? <WorkspaceTradeAnalyzerPanel currency={row.tradeCurrency} direction={row.direction} executionCount={row.executionCount} gainLossDecimal={row.gainLossDecimal} onClose={onClose} open roundTripId={row.roundTripId} symbol={row.symbol} timezone={accountTimezone} /> : null}
  </Box>;
}

export function WorkspaceTradeDrawer({ accountCurrency, accountTimezone, addOpen, addTradeOpen, currentAccountStillMatches, detail, expectedAccountSelectionRef, offlineScopeRef, onAddTradeSaved, onClose, startingTab }: Readonly<{ accountCurrency: string; accountTimezone: string; addOpen?: boolean; addTradeOpen?: boolean; currentAccountStillMatches: () => Promise<boolean>; detail: WorkspaceTradeLibraryRow | null; expectedAccountSelectionRef: string; offlineScopeRef: string; onAddTradeSaved?: () => void; onClose: () => void; startingTab: number }>) {
  const tradeEntryOpen = addOpen ?? addTradeOpen ?? false;
  const open = tradeEntryOpen || detail !== null;
  ...
  useEffect(() => { setSavedTrade(null); setTab(tradeEntryOpen ? 0 : startingTab); }, [detail?.roundTripId, startingTab, tradeEntryOpen]);
  if (!open) return null;
  const activeTrade = savedTrade ?? detail;
  const isNewTrade = tradeEntryOpen && savedTrade === null;
  ...
  return <Drawer anchor="right" onClose={onClose} open={open} slotProps={{ paper: { sx: { width: { xs: "100vw", md: 880 } } } }}><Stack sx={{ height: "100%" }}><Box sx={{ borderBottom: 1, borderColor: "divider", p: 2 }}><Stack direction="row" sx={{ justifyContent: "space-between" }}><Typography component="h2" variant="h6">{isNewTrade ? "Add trade" : activeTrade?.symbol}</Typography><Button onClick={onClose}>Close</Button></Stack><Tabs onChange={(_event, next) => setTab(next)} value={tab} variant="fullWidth"><Tab label="Trade" /><Tab label="Journal" /><Tab label="Analyzer" /></Tabs></Box>{isNewTrade ? <><Box sx={{ display: tab === 0 ? "block" : "none", flex: 1, minHeight: 0, overflowY: "auto", p: 2 }}><AddTradePanel accountCurrency={accountCurrency} accountTimezone={accountTimezone} embedded expectedAccountSelectionRef={expectedAccountSelectionRef} offlineScopeRef={offlineScopeRef} onClose={onClose} onSaved={handleNewTradeSaved} /></Box>{tab === 1 ? <Box sx={{ flex: 1, minHeight: 0, p: 2 }}><Typography>Notes, tags and rules appear here when you add and save a trade.</Typography></Box> : null}{tab === 2 ? <Box sx={{ flex: 1, minHeight: 0, p: 2 }}><Typography>Analysis for qualifying trades will appear here.</Typography></Box> : null}</> : <SavedTradePanel accountCurrency={accountCurrency} accountTimezone={accountTimezone} currentAccountStillMatches={currentAccountStillMatches} expectedAccountSelectionRef={expectedAccountSelectionRef} offlineScopeRef={offlineScopeRef} onClose={onClose} onTabChange={setTab} row={activeTrade} tab={tab} />}</Stack></Drawer>;
}
...
export function WorkspaceTradeLibrary({ accountCurrency, accountTimezone, addTradeOpen, customEndDate, customStartDate, expectedAccountSelectionRef, offlineScopeRef, onAddTradeClose, periodEndDate, periodStartDate, trades }: Readonly<{ accountCurrency: string; accountTimezone: string; addTradeOpen: boolean; customEndDate: string | null; customStartDate: string | null; expectedAccountSelectionRef: string; offlineScopeRef: string; onAddTradeClose: () => void; periodEndDate: string | null; periodStartDate: string | null; trades: WorkspaceTradeLibraryModel }>) {
  ...
  const closeAtomicEdit = () => { setAtomicEditRoundTripId(null); setDetail(null); };
  const actionButtons = (row: WorkspaceTradeLibraryRow) => <Stack direction="row" spacing={0.25}><ActionButton label={`Open chart for ${row.symbol}`} onClick={() => openChart(row.symbol)}><CandlestickChartIcon fontSize="small" /></ActionButton><ActionButton label="Review trade" onClick={() => { setDetail(row); setDetailTab(1); }}><FactCheckOutlinedIcon fontSize="small" /></ActionButton><ActionButton label="Edit trade" onClick={() => { setDetail(null); setAtomicEditRoundTripId(row.roundTripId); }}><EditRoundedIcon fontSize="small" /></ActionButton>{row.tradeDeleteRef ? <ActionButton desktopOnly label="Delete trade" onClick={() => void prepareTableDelete(row, row.tradeDeleteRef!)}><DeleteOutlineRoundedIcon fontSize="small" /></ActionButton> : null}<ActionButton color={row.hasCurrentAnalyzerResult ? "warning.main" : undefined} label={row.hasCurrentAnalyzerResult ? "Open saved Trade Analyzer" : "Open Trade Analyzer"} onClick={() => { setDetail(row); setDetailTab(2); }}><InsightsRoundedIcon fontSize="small" /></ActionButton></Stack>;
  const deleteFromTable = async () => { if (!tableDelete || !(await currentAccountStillMatches())) return; setDeletingTableExecution(true); setTableDeleteError(null); try { const response = await fetch(`/api/platform/journal/workspace-trades/${tableDelete.deleteRef}`, { body: JSON.stringify({ expectedAccountSelectionRef, idempotencyKey: crypto.randomUUID(), roundTripId: tableDelete.roundTripId }), headers: { "Content-Type": "application/json", [JOURNAL_MUTATION_REQUEST_HEADER]: "1" }, method: "DELETE" }); const result = await response.json().catch(() => null) as Readonly<{ code?: unknown }> | null; if (!response.ok) { setTableDeleteError(deleteFailureMessage(result?.code)); return; } setDetail(null); setTableDelete(null); window.location.reload(); } catch { setTableDeleteError("The trade could not be deleted. Refresh the page and try again."); } finally { setDeletingTableExecution(false); } };
