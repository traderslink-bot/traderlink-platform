...
import { hasSharedAnalyzerAllowance } from "@/src/modules/level-analysis/contracts/shared-analyzer-beta-contracts";
...
export function WorkspaceTradeAnalyzerPanel({ currency, direction, executionCount, gainLossDecimal, onClose, open, roundTripId, symbol, timezone }: WorkspaceTradeAnalyzerPanelProps) {
  ...
  const [availability, setAvailability] = useState<SharedAnalyzerAvailability | null>(null);
  ...
  async function requestAnalysis(refreshIndicators = false): Promise<void> {
    ...
    try {
      ...
      else setRequestError(payload.outcome === "retry_limit_reached"
        ? "This trade has used its three free candle-data retries for today. You can try again after midnight New York time."
        : payload.outcome === "usage_exhausted"
      ...
      setRequestError("The analysis could not be requested. Try again.");
