...
export function WorkspaceDashboard({
  ...
  const [savedCurrentFocus, setSavedCurrentFocus] = useState(reviewSummary?.currentFocus ?? null);
  const [savedRuleResultsCardPreference, setSavedRuleResultsCardPreference] = useState(ruleResultsCardPreference ?? { revision: null, showInWorkspace: false });
  const [savedPrScannerCardPreference, setSavedPrScannerCardPreference] = useState(prScannerCardPreference ?? { revision: null, showInWorkspace: true });
  const [savedTopTickersCardPreference, setSavedTopTickersCardPreference] = useState(topTickersCardPreference ?? { revision: null, showInWorkspace: true });
  ...
  useEffect(() => { setSavedCurrentFocus(reviewSummary?.currentFocus ?? null); }, [reviewSummary?.currentFocus]);
  useEffect(() => { const preference = ruleResultsCardPreference ?? { revision: null, showInWorkspace: false }; setSavedRuleResultsCardPreference(preference); setShowRuleResultsCard(preference.showInWorkspace); }, [ruleResultsCardPreference]);
  useEffect(() => { const preference = prScannerCardPreference ?? { revision: null, showInWorkspace: true }; setSavedPrScannerCardPreference(preference); setShowPrScannerCard(preference.showInWorkspace); }, [prScannerCardPreference]);
  useEffect(() => { const preference = topTickersCardPreference ?? { revision: null, showInWorkspace: true }; setSavedTopTickersCardPreference(preference); setShowTopTickersCard(preference.showInWorkspace); }, [topTickersCardPreference]);
    ...
    <DashboardChartProvider>
      ...
      {hasLiveTradeLibraryProps(tradeLibraryProps) ? (
        <>
          ...
          <WorkspaceMoreFiltersDrawer customEndDate={tradeLibraryProps.customEndDate} customStartDate={tradeLibraryProps.customStartDate} expectedAccountSelectionRef={tradeLibraryProps.expectedAccountSelectionRef} initialFocus={savedCurrentFocus} newsScannerAvailable={newsScannerAvailable} onClose={() => setFiltersOpen(false)} onPreferenceSaved={(kind, show, preference, focus) => { if (kind === "rules" && preference) { setSavedRuleResultsCardPreference(preference); setShowRuleResultsCard(show); } else if (kind === "scanner" && preference) { setSavedPrScannerCardPreference(preference); setShowPrScannerCard(show); } else if (kind === "tickers" && preference) { setSavedTopTickersCardPreference(preference); setShowTopTickersCard(show); } else if (kind === "focuses" && focus) { setSavedCurrentFocus(focus); setCurrentFocuses(show && focus.focusText.trim() ? focus.focusText.trim() : null); } }} open={filtersOpen} prScannerPreference={savedPrScannerCardPreference} query={tradeLibraryProps.trades.query} ruleResultsPreference={savedRuleResultsCardPreference} topTickersPreference={savedTopTickersCardPreference} />
          <JournalNotesDrawer expectedAccountSelectionRef={tradeLibraryProps.expectedAccountSelectionRef} focusOnly={sessionNotesInitialView === "focuses"} initialView={sessionNotesInitialView} key={sessionNotesInitialView} launch={{ kind: "session", sessionDate: sessionDateInTimezone(tradeLibraryProps.accountTimezone) }} onClose={() => setSessionNotesOpen(false)} onFocusSaved={(focus) => { setSavedCurrentFocus(focus); setCurrentFocuses(focus.showInWorkspace && focus.focusText.trim() ? focus.focusText.trim() : null); }} open={sessionNotesOpen} />
          {rulesOpen && rulesAccountSelectionRef === activeAccountSelectionRef ? <WorkspaceRulesPanel initialView={rulesInitialView} key={rulesInitialView} onClose={() => setRulesOpen(false)} onPreferenceSaved={(preference) => { setSavedRuleResultsCardPreference(preference); setShowRuleResultsCard(preference.showInWorkspace); }} /> : null}
