...
export default async function CommunityWatchlistDetailPage({
  ...
  return <DashboardPage><Stack spacing={1.75} sx={{ maxWidth: 1040 }}><Paper elevation={0} sx={{ border: 1, borderColor: "divider", borderRadius: 2.5, overflow: "hidden" }}><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "minmax(0, 1fr) 224px" } }}><Stack spacing={1.5} sx={{ minWidth: 0, p: { xs: 1.75, sm: 2.25 } }}><Stack direction={{ xs: "column", sm: "row" }} spacing={1.25} sx={{ alignItems: { sm: "flex-start" }, justifyContent: "space-between" }}><Box sx={{ minWidth: 0 }}><Stack direction="row" spacing={1} sx={{ alignItems: "center", flexWrap: "wrap", rowGap: 0.75 }}><Typography component="h1" variant="h1">{detail.title}</Typography><Chip label={`${detail.symbolCount} ${detail.symbolCount === 1 ? "symbol" : "symbols"}`} size="small" sx={{ bgcolor: "#edf3ff", color: "#082b73", fontWeight: 800 }} /></Stack>{detail.description ? <Typography color="text.secondary" sx={{ mt: 0.75 }} variant="body2">{detail.description}</Typography> : null}</Box></Stack>{detail.tags.length ? <Stack direction="row" spacing={0.65} sx={{ flexWrap: "wrap", rowGap: 0.65 }}>{detail.tags.map((tag, index) => <Chip key={tag} label={tag} size="small" sx={{ bgcolor: index % 2 ? "#e9f7ef" : "#edf3ff", color: index % 2 ? "#14663c" : "#082b73", fontWeight: 700 }} />)}</Stack> : null}<CommunityWatchlistTickerBoard detail={detail} editable={editable} tickerFacts={tickerFacts} watchlistSlug={watchlistName} /></Stack><Stack spacing={1.15} sx={{ alignItems: "center", bgcolor: "#f8faff", borderColor: "divider", borderLeft: { xs: 0, lg: 1 }, borderTop: { xs: 1, lg: 0 }, justifyContent: "center", p: 2.25, textAlign: "center" }}><Avatar sx={{ bgcolor: "#102b69", fontWeight: 850, height: 64, width: 64 }}>{avatarLetters(detail.authorHandle)}</Avatar><Box><Typography sx={{ fontWeight: 850 }} variant="h3">@{detail.authorHandle}</Typography><Typography color="text.secondary" variant="body2">Community trader</Typography></Box><Typography color="text.secondary" variant="body2">Sharing small-cap research and watchlist ideas.</Typography><Link href={`/community/${detail.authorHandle}`} style={{ textDecoration: "none" }}><DashboardSecondaryAction component="span">View profile</DashboardSecondaryAction></Link></Stack></Box></Paper></Stack></DashboardPage>;
}
