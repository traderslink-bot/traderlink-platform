...
export async function shareWatchlistWithCommunityAction(formData:FormData):Promise<void>{const identity=await requireTraderLinkPlatformPageIdentity();const target=String(formData.get("target")??"").split("|");const [communityId,audienceId,communitySlug]=target;const watchlistId=String(formData.get("watchlistId")??"");if(!communityId||!audienceId||!communitySlug||!watchlistId)throw new Error("Choose a watchlist and community.");withPlatformDatabase({mode:"runtime"},database=>{const actor=resolveTraderLinkCommunityViewer(database,identity,communitySlug);new TraderLinkCommunityPlatformRepository(database).shareWatchlist({communityId,actor,watchlistId,audienceId,atUtc:createCanonicalUtcTimestamp()});});revalidatePath(`/communities/${communitySlug}`);redirect(`/communities/${communitySlug}/watchlists`);}
