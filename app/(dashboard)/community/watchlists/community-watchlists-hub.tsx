  ...
  return <Stack spacing={0.65}>
    ...
    <Stack direction="row" spacing={0.75} sx={{ alignItems: "stretch", flexWrap: "wrap", rowGap: 0.75 }}>
      {shownSymbols.map((symbol, index) => {
        ...
        return <Box key={symbol} sx={{ bgcolor: isPreview ? "#edf3ff" : "#fafbfe", border: 1, borderColor: isPreview ? "#b9cdf7" : "divider", borderRadius: 1.5, minWidth: { xs: 68, sm: 76 }, px: 1, py: 0.75 }}>
          <Typography sx={{ color: "#082b73", fontSize: "0.82rem", fontWeight: 850, lineHeight: 1.1 }}>{symbol}</Typography>
      ...
      {item.symbolCount > shownSymbols.length ? <Box sx={{ alignItems: "center", bgcolor: "#fafbfe", border: 1, borderColor: "divider", borderRadius: 1.5, display: "flex", fontSize: "0.78rem", fontWeight: 800, justifyContent: "center", minWidth: 62, px: 1 }}>+{item.symbolCount - shownSymbols.length}</Box> : null}
    </Stack>
  </Stack>;
}
  ...
  return <Box sx={{ bgcolor: "#fbfcff", border: 1, borderColor: "#d7e2f8", borderRadius: 1.75, px: { xs: 1.25, md: 1.5 }, py: 1.25 }}>
    <Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ alignItems: { sm: "center" }, justifyContent: "space-between", mb: facts.length ? 1.25 : 0 }}>
      <Stack direction="row" spacing={0.8} sx={{ alignItems: "center", minWidth: 0 }}>
        <Typography sx={{ color: "#082b73", fontWeight: 900 }} variant="body2">{ticker.symbol}</Typography>
        <Typography color="text.secondary" variant="caption">Ticker preview</Typography>
        {ticker.tags.slice(0, 2).map((tag, index) => <Chip key={tag} label={tag} size="small" sx={{ ...tagTone(index), fontSize: "0.69rem", fontWeight: 700, height: 22 }} />)}
      </Stack>
    </Stack>
    ...
    <Box sx={{ display: "grid", gap: 0.75, gridTemplateColumns: { xs: "1fr", sm: "repeat(2, minmax(0, 1fr))" } }}>
      {item.tickerPreviews.map((ticker) => {
        ...
        return <Button key={ticker.symbol} onClick={() => setActiveSymbol(ticker.symbol)} sx={{ alignItems: "stretch", border: 1, borderColor: active ? "primary.main" : "divider", borderRadius: 1.5, color: "text.primary", display: "block", minHeight: 68, p: 1, textAlign: "left", textTransform: "none" }} variant="text">
          <Stack spacing={0.35}>
            <Stack direction="row" spacing={0.75} sx={{ alignItems: "center", justifyContent: "space-between" }}><Typography sx={{ color: "#082b73", fontWeight: 900 }} variant="body2">{ticker.symbol}</Typography><Typography color="text.secondary" sx={{ fontSize: "0.68rem" }}>{detailCount ? `${detailCount} note${detailCount === 1 ? "" : "s"}` : "No notes"}</Typography></Stack>
            <Stack direction="row" spacing={0.5} sx={{ flexWrap: "wrap", rowGap: 0.5 }}>{ticker.tags.slice(0, 2).map((tag, index) => <Chip key={tag} label={tag} size="small" sx={{ ...tagTone(index), fontSize: "0.64rem", fontWeight: 700, height: 20 }} />)}</Stack>
...
function SelectedTraderCard({ item }: { item: CommunityWatchlistSummary }) {
  return <Paper elevation={0} sx={{ alignSelf: "start", border: 1, borderColor: "divider", borderRadius: 2.5, p: 2.5, textAlign: "center" }}><Stack spacing={1.3} sx={{ alignItems: "center" }}><Avatar sx={{ bgcolor: "#102b69", fontSize: "1.05rem", fontWeight: 850, height: 76, width: 76 }}>{avatarLetters(item.authorHandle)}</Avatar><Box><Typography sx={{ fontWeight: 850 }} variant="h3">@{item.authorHandle}</Typography><Typography color="text.secondary" variant="body2">Community trader</Typography></Box><Typography color="text.secondary" variant="body2">Sharing small-cap research and watchlist ideas.</Typography><Button component={Link} href={`/community/${item.authorHandle}`} variant="outlined">View profile</Button></Stack></Paper>;
}
