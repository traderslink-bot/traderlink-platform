...
import {
  addCommunityWatchlistTicker,
  setCommunityWatchlistFollow,
  updateCommunityWatchlistDescription,
  updateCommunityWatchlistTickerTake,
  updateCommunityWatchlistTickerTags,
} from "./community-watchlist-actions";

const communityWatchlistFactColumns = {
  xs: "repeat(2, minmax(0, 1fr))",
  sm: "124px 76px 78px 62px 150px minmax(0, 1fr)",
};
...
export function CommunityWatchlistCard({
  ...
  const [descriptionDraft, setDescriptionDraft] = useState(detail.description);
  const [savingDescription, setSavingDescription] = useState(false);
  const [editorMessage, setEditorMessage] = useState<string | null>(null);
  const router = useRouter();
  const visibleTickerSymbols = detail.tickers.slice(0, 5);
  ...
  const headerClick = (event: MouseEvent<HTMLElement>) => {
    if (!(event.target as HTMLElement).closest("a, button, input, textarea, [role='combobox']")) toggle();
  };
  const headerKeyDown = (event: KeyboardEvent<HTMLElement>) => {
    if ((event.key === "Enter" || event.key === " ") && !(event.target as HTMLElement).closest("a, button, input, textarea, [role='combobox']")) {
  ...
  return <Paper elevation={0} sx={{ border: 0, borderRadius: 2.5, overflow: "hidden" }}>
    <Box aria-expanded={expanded} onClick={headerClick} onKeyDown={headerKeyDown} role="button" sx={{ cursor: "pointer", display: "grid", gridTemplateColumns: { xs: "1fr", lg: "minmax(0, 1fr) 260px" } }} tabIndex={0}>
      <Stack spacing={1.25} sx={{ minWidth: 0, p: { xs: 1.75, sm: 2.25 } }}>
        <Box sx={{ minWidth: 0 }}>
          <Stack direction="row" spacing={1} sx={{ alignItems: "center", flexWrap: "wrap", rowGap: 0.75 }}>
            ...
            {detail.status === "published" && !editable ? <Button disabled={savingFollow} onClick={async () => {
            ...
            }} size="small" sx={{ borderColor: "#082b73", borderRadius: "6px", fontSize: "0.74rem", fontWeight: 800, lineHeight: 1.1, minHeight: 28, px: 1, py: 0.3, textTransform: "none", whiteSpace: "nowrap" }} variant="outlined">{savingFollow ? "Saving..." : following ? "Unfollow Watchlist" : "Follow Watchlist"}</Button> : null}
          </Stack>
          {editing ? <Stack direction={{ xs: "column", sm: "row" }} spacing={0.75} sx={{ alignItems: { sm: "flex-start" }, mt: 0.75 }}>
            <TextField fullWidth helperText={`${descriptionDraft.length}/180`} label="Description" minRows={2} multiline onChange={(event) => setDescriptionDraft(event.target.value)} onClick={(event) => event.stopPropagation()} slotProps={{ htmlInput: { maxLength: 180 } }} value={descriptionDraft} />
            <Button disabled={savingDescription || descriptionDraft === detail.description} onClick={async (event) => { event.stopPropagation(); setSavingDescription(true); setEditorMessage(null); const result = await updateCommunityWatchlistDescription({ handle: detail.authorHandle, watchlistSlug, description: descriptionDraft }); setSavingDescription(false); setEditorMessage(result.message); if (result.ok) router.refresh(); }} size="small" sx={{ minWidth: 114, mt: { sm: 0.25 } }} variant="outlined">{savingDescription ? "Saving..." : "Save description"}</Button>
          </Stack> : detail.description ? <Typography color="text.secondary" sx={{ mt: 0.75 }} variant="body2">{detail.description}</Typography> : null}
          {followMessage ? <Typography color="error" role="status" sx={{ display: "block", mt: 0.6 }} variant="caption">{followMessage}</Typography> : null}
          {editorMessage ? <Typography color="text.secondary" role="status" sx={{ display: "block", mt: 0.6 }} variant="caption">{editorMessage}</Typography> : null}
        </Box>
        ...
        <Stack direction="row" spacing={0.65} sx={{ alignItems: "center", flexWrap: "wrap", rowGap: 0.65 }}>
          {visibleTickerSymbols.map((ticker) => <Chip key={ticker.symbol} label={ticker.symbol} size="small" sx={{ bgcolor: "#e9f7ef", color: "#14663c", fontWeight: 850 }} />)}
          {hiddenTickerCount > 0 ? <Chip label={`${hiddenTickerCount} more`} onClick={(event) => { event.stopPropagation(); toggle(); }} size="small" sx={{ borderColor: "#b7cae8", color: "#082b73", fontWeight: 800 }} variant="outlined" /> : null}
        ...
        <Typography sx={{ color: "#7a8495", fontSize: "0.72rem", fontStyle: "italic", fontWeight: 500 }} variant="caption">{watchlistUpdateText(detail.updatedAtUtc)}</Typography>
      </Stack>
      <Stack spacing={0.65} sx={{ alignItems: "center", alignSelf: { xs: "stretch", lg: "start" }, boxShadow: "inset 3px 0 0 #082b73", minWidth: 0, p: { xs: 1.25, sm: 1.5 }, textAlign: "center" }}>
        <Avatar sx={{ bgcolor: "#102b69", fontWeight: 850, height: 48, width: 48 }}>{avatarLetters(detail.authorDisplayName)}</Avatar>
        <Typography noWrap sx={{ fontSize: "0.78rem", fontWeight: 850, maxWidth: "100%" }}>@{detail.authorDisplayName}</Typography>
        {detail.authorDescription ? <Typography sx={{ color: "text.secondary", fontSize: "0.72rem", lineHeight: 1.35, maxWidth: "100%" }} variant="body2">{detail.authorDescription}</Typography> : null}
        ...
        {editable ? <Button onClick={(event) => { event.stopPropagation(); setExpanded(true); setEditing((current) => !current); }} size="small" sx={{ borderColor: "#082b73", fontSize: "0.72rem", fontWeight: 800, minHeight: 28, mt: 0.7, px: 0.9, textTransform: "none" }} variant="outlined">{editing ? "Done editing" : "Edit watchlist"}</Button> : null}
      </Stack>
    </Box>
    {expanded ? <Box sx={{ p: { xs: 1.75, sm: 2.25 } }}><CommunityWatchlistTickerBoard detail={detail} editable={editable && editing} tickerFacts={tickerFacts} watchlistSlug={watchlistSlug} /></Box> : null}
  </Paper>;
}
...
export function CommunityWatchlistTickerBoard({
  ...
  const [tickerTakeDrafts, setTickerTakeDrafts] = useState<Readonly<Record<string, string>>>({});
  ...
  const [savingNewTicker, setSavingNewTicker] = useState(false);
  const [newTickerMessage, setNewTickerMessage] = useState<string | null>(null);
  const [loadedTickerFacts, setLoadedTickerFacts] = useState(tickerFacts);
  const router = useRouter();

  useEffect(() => {
    if (Object.keys(tickerFacts).length || !detail.tickers.length) return;
    let active = true;
    void fetch(`/api/community/ticker-facts?symbols=${encodeURIComponent(detail.tickers.map((ticker) => ticker.symbol).join(","))}`)
      .then(async (response) => response.ok ? response.json() as Promise<Readonly<Record<string, CommunityTickerCompanyFacts | null>>> : {})
      .then((facts) => { if (active) setLoadedTickerFacts(facts); })
      .catch(() => undefined);
    return () => { active = false; };
  }, [detail.tickers, tickerFacts]);
  ...
  return <Stack spacing={0.65}>
    {showFactLabels && Object.values(loadedTickerFacts).some(Boolean) ? <Box aria-hidden="true" sx={{ border: 1, borderColor: "transparent", boxSizing: "border-box", columnGap: 0.65, display: { xs: "none", sm: "grid" }, gridTemplateColumns: communityWatchlistFactColumns.sm, justifyItems: "start", px: 1.5 }}>
      <Box />
      {['M/C', 'O/S', 'Country', 'Industry'].map((label) => <Typography color="text.secondary" key={label} sx={{ fontSize: "0.72rem", fontWeight: 900, letterSpacing: ".03em" }}>{label}</Typography>)}
      <Box />
    </Box> : null}
    <Stack spacing={0.7}>
      {detail.tickers.map((ticker) => {
        ...
        const companyFacts = loadedTickerFacts[ticker.symbol] ?? null;
        const facts = companyFacts ? [
          { label: "M/C", value: companyFacts.marketCap },
          { label: "O/S", value: companyFacts.sharesOutstanding },
          { label: "Country", value: companyFacts.country },
          { label: "Industry", value: companyFacts.industry },
        ] : [];
        const factRows = [
          ...
          <Button aria-expanded={active} onClick={() => setSelectedSymbol((current) => current === ticker.symbol ? "" : ticker.symbol)} sx={{ alignItems: "center", bgcolor: "#edf4ff", border: 1, borderColor: active ? "#9fbee9" : "#d5e3fb", borderRadius: 1.75, boxShadow: "inset 3px 0 0 #082b73", boxSizing: "border-box", color: "text.primary", columnGap: { xs: 0.75, sm: 0.65 }, display: "grid", gridTemplateColumns: communityWatchlistFactColumns, justifyItems: "start", minHeight: { xs: 78, sm: 58 }, px: { xs: 1.25, sm: 1.5 }, py: { xs: 1, sm: 0.85 }, textAlign: "left", textTransform: "none", width: "100%", "&:hover": { bgcolor: "#e7f0ff" } }} variant="text">
            <Stack spacing={0.45} sx={{ gridColumn: { xs: "1 / -1", sm: "auto" }, minWidth: 0 }}>
              <Stack direction="row" spacing={0.55} sx={{ alignItems: "center" }}>
                <Typography sx={{ color: "#14663c", fontSize: "1rem", fontWeight: 900, letterSpacing: ".01em", lineHeight: 1.1 }}>{ticker.symbol}</Typography>
            ...
            </Box>) : <Box aria-hidden="true" sx={{ display: { xs: "none", sm: "block" }, gridColumn: "2 / span 4" }} />}
          ...
          {active ? <Box sx={{ bgcolor: "#fbfcff", border: 1, borderColor: "#c9daf7", borderRadius: "0 0 10px 10px", borderTop: 0, boxShadow: "inset 3px 0 0 #082b73", mt: -0.85, p: { xs: 1.5, sm: 1.75 }, pt: { xs: 2.2, sm: 2.35 } }}>
            <Box sx={{ display: "grid", gap: { xs: 1.5, md: 2 }, gridTemplateColumns: { xs: "1fr", md: "minmax(0, 1.55fr) minmax(190px, .65fr)" } }}>
              <Box sx={{ minWidth: 0 }}>
                ...
                {editable ? <Stack spacing={0.75} sx={{ mt: 0.85 }}>
                  <TextField fullWidth helperText={`${(tickerTakeDrafts[ticker.symbol] ?? note).length}/800`} label="Trader's take" minRows={3} multiline onChange={(event) => setTickerTakeDrafts((current) => ({ ...current, [ticker.symbol]: event.target.value }))} slotProps={{ htmlInput: { maxLength: 800 } }} value={tickerTakeDrafts[ticker.symbol] ?? note} />
                  <Stack direction="row" spacing={0.75} sx={{ alignItems: "center" }}><Button disabled={savingTake || (tickerTakeDrafts[ticker.symbol] ?? note) === note} onClick={async () => { setSavingTake(true); setTickerMessage(null); const result = await updateCommunityWatchlistTickerTake({ handle: detail.authorHandle, watchlistSlug, symbol: ticker.symbol, take: tickerTakeDrafts[ticker.symbol] ?? note }); setSavingTake(false); setTickerMessage(result.message); if (result.ok) router.refresh(); }} size="small" variant="outlined">{savingTake ? "Saving..." : "Save Trader's take"}</Button></Stack>
                </Stack> : <Typography sx={{ fontSize: "0.9rem", lineHeight: 1.65, mt: 0.55, whiteSpace: "pre-wrap" }} variant="body2">{note || "No trader&apos;s take was added for this ticker yet."}</Typography>}
              </Box>
            ...
            </Box> : ticker.tags.length ? <Stack direction="row" spacing={0.55} sx={{ borderColor: "divider", borderTop: 1, flexWrap: "wrap", mt: 1.5, pt: 1.25, rowGap: 0.55, width: "100%" }}>{ticker.tags.map((tag) => <Chip key={tag} label={tag} size="small" sx={{ borderColor: "#b7cae8", color: "#082b73", fontWeight: 700 }} variant="outlined" />)}</Stack> : null}
          </Box> : null}
        </Box>;
      })}
    </Stack>
    {editable ? <Stack direction={{ xs: "column", sm: "row" }} spacing={0.75} sx={{ alignItems: { sm: "center" }, borderColor: "divider", borderTop: 1, mt: 0.8, pt: 1.25 }}>
      <TextField label="Ticker symbol" onChange={(event) => setNewTickerSymbol(event.target.value.toUpperCase())} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); } }} placeholder="Add a ticker" size="small" slotProps={{ htmlInput: { maxLength: 12 } }} value={newTickerSymbol} />
      <Button disabled={savingNewTicker || !newTickerSymbol.trim()} onClick={async () => { setSavingNewTicker(true); setNewTickerMessage(null); const result = await addCommunityWatchlistTicker({ handle: detail.authorHandle, watchlistSlug, symbol: newTickerSymbol }); setSavingNewTicker(false); setNewTickerMessage(result.message); if (result.ok) { setNewTickerSymbol(""); router.refresh(); } }} size="small" variant="outlined">{savingNewTicker ? "Adding..." : "Add ticker"}</Button>
      {newTickerMessage ? <Typography color="text.secondary" role="status" variant="caption">{newTickerMessage}</Typography> : null}
    </Stack> : null}
  </Stack>;
}
