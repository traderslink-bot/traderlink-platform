    ...
    <Stack spacing={0.7}>
      {detail.tickers.map((ticker) => {
        ...
        return <Box key={ticker.symbol}>
          ...
          {active ? <Box sx={{ bgcolor: "#fbfcff", border: 1, borderColor: "#c9daf7", borderRadius: "0 0 10px 10px", borderTop: 0, boxShadow: "inset 3px 0 0 #082b73", mt: -0.85, p: { xs: 1.5, sm: 1.75 }, pt: { xs: 2.2, sm: 2.35 } }}><Box sx={{ display: "grid", gap: { xs: 1.5, md: 2 }, gridTemplateColumns: { xs: "1fr", md: "minmax(0, 1.55fr) minmax(190px, .65fr)" } }}><Box sx={{ minWidth: 0 }}><Typography color="text.secondary" sx={{ fontWeight: 800 }} variant="caption">Trader&apos;s take</Typography><Typography sx={{ fontSize: "0.9rem", lineHeight: 1.65, mt: 0.55, whiteSpace: "pre-wrap" }} variant="body2">{note || "No trader&apos;s take was added for this ticker yet."}</Typography>{editable ? <Stack spacing={1.15} sx={{ mt: 1.5 }}><Stack direction={{ xs: "column", sm: "row" }} spacing={0.75} sx={{ alignItems: { sm: "center" } }}><input aria-label={`Replace ${ticker.symbol} with a ticker`} onChange={(event) => setTickerEdit(event.target.value.toUpperCase())} placeholder="New ticker" style={{ border: "1px solid #bac8e0", borderRadius: 6, font: "inherit", minHeight: 34, padding: "6px 9px", textTransform: "uppercase", width: 130 }} value={tickerEdit} /><Button disabled={savingTicker || !tickerEdit.trim()} onClick={async () => { setSavingTicker(true); setTickerMessage(null); const result = await replaceCommunityWatchlistTicker({ handle: detail.authorHandle, watchlistSlug, currentSymbol: ticker.symbol, nextSymbol: tickerEdit }); setSavingTicker(false); setTickerMessage(result.message); if (result.ok) { setTickerEdit(""); router.refresh(); } }} size="small" variant="outlined">{savingTicker ? "Saving..." : "Change ticker"}</Button></Stack><Box><Typography color="text.secondary" sx={{ fontWeight: 800 }} variant="caption">Ticker tags</Typography><Stack direction="row" spacing={0.55} sx={{ flexWrap: "wrap", mt: 0.65, rowGap: 0.55 }}>{tagChoices.map((tag) => <Chip color={selectedTags.includes(tag) ? "primary" : "default"} key={tag} label={tag} onClick={() => setPendingTickerTags((current) => ({ ...current, [ticker.symbol]: selectedTags.includes(tag) ? selectedTags.filter((item) => item !== tag) : [...selectedTags, tag] }))} size="small" variant={selectedTags.includes(tag) ? "filled" : "outlined"} />)}</Stack><Stack direction={{ xs: "column", sm: "row" }} spacing={0.75} sx={{ alignItems: { sm: "center" }, mt: 0.9 }}><TextField label="New tag" onChange={(event) => setNewTickerTag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addCustomTickerTag(ticker.symbol, selectedTags); } }} size="small" slotProps={{ htmlInput: { maxLength: 48 } }} value={newTickerTag} /><Button disabled={savingTicker || !newTickerTag.trim() || selectedTags.length >= 20} onClick={() => addCustomTickerTag(ticker.symbol, selectedTags)} size="small" variant="outlined">Add</Button><Button disabled={savingTicker || sameTags(selectedTags, ticker.tags)} onClick={async () => { setSavingTicker(true); setTickerMessage(null); const result = await updateCommunityWatchlistTickerTags({ handle: detail.authorHandle, watchlistSlug, symbol: ticker.symbol, tags: selectedTags }); setSavingTicker(false); setTickerMessage(result.message); if (result.ok) { setPendingTickerTags((current) => ({ ...current, [ticker.symbol]: selectedTags })); setNewTickerTag(""); router.refresh(); } }} size="small" variant="outlined">{savingTicker ? "Saving..." : "Save tags"}</Button></Stack></Box>{tickerMessage ? <Typography color="text.secondary" variant="caption">{tickerMessage}</Typography> : null}</Stack> : null}</Box><Stack spacing={1.1} sx={{ alignSelf: "start", borderColor: "divider", borderLeft: { xs: 0, md: 1 }, borderTop: { xs: 1.25, md: 0 }, minWidth: 0, pl: { xs: 0, md: 1.75 }, pt: { xs: 1.25, md: 0 } }}>{factRows.length ? factRows.map((fact) => <Box key={fact.label}><Typography color="text.secondary" sx={{ fontWeight: 800 }} variant="caption">{fact.label}</Typography><Typography sx={{ fontSize: "0.88rem", fontWeight: 700, lineHeight: 1.45 }} variant="body2">{fact.value}</Typography></Box>) : <Typography color="text.secondary" variant="body2">No target or catalyst was added.</Typography>}</Stack></Box>{ticker.tags.length ? <Stack direction="row" spacing={0.55} sx={{ borderColor: "divider", borderTop: 1, flexWrap: "wrap", mt: 1.5, pt: 1.25, rowGap: 0.55 }}>{ticker.tags.map((tag) => <Chip key={tag} label={tag} size="small" variant="outlined" />)}</Stack> : null}</Box> : null}
        </Box>;
      })}
    </Stack>
  </Stack>;
