...
type ExecutionFeedbackItem = {
  ...
  summary: {
    ...
    points: {
      context: FeedbackPoint[];
      strengths: FeedbackPoint[];
      risks: FeedbackPoint[];
      primaryFocus: FeedbackPoint | null;
    };
    warnings: string[];
    limitations: string[];
  } | null;
...
type ExecutionFeedbackBatchResponse = {
  ...
  items: ExecutionFeedbackItem[];
};

type ApiState =
  | { status: "idle"; body: null; error: null }
  | { status: "loading"; body: null; error: null }
  | { status: "success"; body: ExecutionFeedbackBatchResponse; error: null }
      ...
      <div className="mt-2 flex flex-col gap-2">
        {points.length === 0 ? (
          ...
          points.map((point) => (
            <div
              key={point.id}
              className="border-l border-zinc-700 pl-3 text-sm"
            >
              ...
              <div className="mt-1 text-zinc-400">{point.summary}</div>
            </div>
          ))
        )}
      </div>
...
function ResultItem({ item }: { item: ExecutionFeedbackItem }) {
  return (
    <div className="border-t border-zinc-800 py-5">
      ...
      {item.failure ? (
        <div className="mt-4 border-l-2 border-red-600 pl-4 text-sm">
          ...
          <div className="mt-1 text-zinc-400">{item.failure.message}</div>
        </div>
      ) : null}
          ...
          <div className="grid gap-4 text-sm lg:grid-cols-4">
            <div>
              ...
              <div className="mt-1 text-zinc-100">
                {item.summary.lifecycle.closedToFlat ? "flat" : "open"} / max{" "}
                {item.summary.lifecycle.maxPositionSize}
              </div>
              <div className="text-zinc-500">
                final {item.summary.lifecycle.finalPositionSize}
              </div>
            </div>
            <div>
              <div className="text-xs uppercase tracking-wide text-zinc-600">
              ...
              <div className="mt-1 text-zinc-100">
                {item.summary.lifecycle.addCountAfterInitialEntry} adds
              </div>
              <div className="text-zinc-500">
                {item.summary.lifecycle.reductionCount} reductions
              </div>
            </div>
            <div>
              <div className="text-xs uppercase tracking-wide text-zinc-600">
              ...
              <div className="mt-1 text-zinc-100">
                {formatNumber(item.summary.executionOnlyPnl.grossRealizedPnl)}
              </div>
              <div className="text-zinc-500">
                {formatNumber(
                  item.summary.executionOnlyPnl
            ...
            <div>
              ...
              <div className="mt-1 text-zinc-100">
                {item.summary.points.primaryFocus?.label ?? "None"}
              </div>
              <div className="break-all font-mono text-xs text-zinc-500">
                {item.summary.points.primaryFocus?.id ?? "no_focus"}
              </div>
            </div>
          </div>

          <div className="grid gap-5 lg:grid-cols-3">
            <PointList title="Risks" points={item.summary.points.risks} />
            <PointList title="Strengths" points={item.summary.points.strengths} />
            <PointList title="Context" points={item.summary.points.context} />
          </div>
        </div>
      ) : null}
    </div>
...
export function ExecutionFeedbackDebugClient() {
  ...
  async function submitRequest() {
    ...
    try {
      const response = await fetch("/api/execution-feedback/debug", {
        method: "POST",
        headers: {
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="flex min-h-[620px] flex-col">
            <label
              ...
              htmlFor="execution-request-json"
            >
              Request JSON
            </label>
            <textarea
              id="execution-request-json"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 px-4 py-3">
              ...
              {apiState.status === "success" ? (
                <div className="text-right text-xs text-zinc-500">
                  ...
                  <div>
                    {apiState.body.pointCounts.risks} risks /{" "}
                    {apiState.body.pointCounts.strengths} strengths
                  </div>
                </div>
              ) : null}
            </div>
