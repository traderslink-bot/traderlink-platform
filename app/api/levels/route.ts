...
export async function POST(request: Request): Promise<Response> {
  ...
  try { body = await request.json() as { symbol?: unknown }; } catch { return Response.json({ state: "unavailable", code: "invalid_symbol", message: "Enter a Nasdaq or NYSE stock ticker." }, { status: 400 }); }
  try {
