...
function finish(request: NextRequest, status: "connected" | "failed" | "invalid-state"): NextResponse {
  const response = NextResponse.redirect(new URL(`/account?moomoo=${status}`, request.nextUrl.origin));
