...
import { parseCoachAiReviewDeliveryConfirmBody, parseCoachAiReviewDeliveryDraftId } from "@/src/modules/coach/server/coach-ai-review-delivery-change-route-runtime";
...
export async function POST(request: Request, { params }: Context): Promise<Response> {
  try {
    ...
    const input = parseCoachAiReviewDeliveryConfirmBody(await parseJsonBody(request));
    const draft = withPlatformDatabase({ mode: "runtime" }, (database) =>
      new CoachAiReviewDeliveryChangeCommandService(database).confirm(scope, {
        conversationId, draftId, editedProposal: input.editedProposal,
      }));
