...
const productOverviews: Record<string, { href: string; label: string }> = {
  "/help/daily-trade-tracker": { href: "/trading-journal", label: "Trading Journal overview" },
  "/help/core-analytics": { href: "/trade-analytics", label: "Trade Analytics overview" },
  "/help/trade-analyzer": { href: "/trade-analyzer", label: "Trade Analyzer overview" },
};

function ProductOverviewLink({ collectionHref }: { collectionHref: string }) {
  const overview = productOverviews[collectionHref];
  if (!overview) return null;
  return <Box sx={{ mt: 3 }}><Link href={`${publicOrigin}${overview.href}`}>{overview.label}</Link></Box>;
}
...
function HelpBlock({ block }: { block: HelpArticleBlock }) {
  ...
  if (block.kind === "link") return <Box><Typography color="text.secondary">{block.text}</Typography><Button endIcon={<ArrowForwardRoundedIcon />} href={destinationHref(block.href)} sx={{ mt: 1 }} variant="outlined">{block.label}</Button></Box>;
  return <Paper variant="outlined" sx={{ maxWidth: "100%" }}><TableContainer><Table size="small" sx={{ minWidth: Math.max(520, 150 + Math.max(0, block.columns.length - 1) * 220) }}><TableHead><TableRow>{block.columns.map((column) => <TableCell key={column} sx={{ bgcolor: "rgba(1, 30, 86, 0.045)", fontWeight: 850 }}>{column}</TableCell>)}</TableRow></TableHead><TableBody>{block.rows.map((row, rowIndex) => <TableRow key={`${rowIndex}-${row[0]}`}>{row.map((cell, cellIndex) => <TableCell key={`${cellIndex}-${cell}`} sx={{ color: cellIndex === 0 ? "text.primary" : "text.secondary", fontWeight: cellIndex === 0 ? 750 : 400, minWidth: cellIndex === 0 ? 150 : 220, verticalAlign: "top" }}>{cell}</TableCell>)}</TableRow>)}</TableBody></Table></TableContainer></Paper>;
}
