...
const actionDraftSchema = z.discriminatedUnion("kind", [
  ...
  z.object({
    kind: z.literal("ai_review_request"),
    reviewKind: z.enum(["weekly", "two_week", "monthly"]),
    periodStartDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/u),
    periodEndDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/u),
  }).strict(),
  z.object({
