...
const actionDraftSchema = z.discriminatedUnion("kind", [
  ...
  z.object({
    kind: z.literal("create_journal_account"),
    displayName: z.string().trim().min(1).max(120),
    baseCurrency: z.string().regex(/^[A-Z]{3}$/u),
    tradingTimezone: z.string().trim().min(1).max(120),
  }).strict(),
  z.object({
    kind: z.literal("swing_note"),
    positionRef: z.string().regex(/^[0-9a-f]{64}$/u),
    reviewDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/u),
    note: z.string().min(1).max(12_000),
    nextSessionPlan: z.string().max(12_000).nullable(),
  }).strict(),
  z.object({
    kind: z.literal("trade_style"),
    positionRef: z.string().regex(/^[0-9a-f]{64}$/u),
  ...
  z.object({
    ...
    roundTripId: z.string().uuid().nullable(),
    positionRef: z.string().regex(/^[0-9a-f]{64}$/u).nullable(),
