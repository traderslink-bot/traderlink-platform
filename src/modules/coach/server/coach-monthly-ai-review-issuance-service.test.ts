...
) VALUES (?, 'development_local', 'coach-monthly-review-test-owner', 'Test owner', 'active', ?, ?)`).run(
...
function input(): CoachMonthlyAiReviewInput {
  return Object.freeze({
...
function output(): CoachMonthlyAiReviewOutput {
  return Object.freeze({
    contractVersion: COACH_MONTHLY_AI_OUTPUT_CONTRACT_VERSION,
    monthlyReview: "A factual monthly review.",
    progressAcrossMonth: "The saved record supports one improvement.",
    recurringFriction: "The saved record supports one limitation.",
    focusFollowThrough: "No Current Focus was available.",
    nextMonthFocuses: Object.freeze(["Keep the next focus specific."]),
...
describe("Coach monthly AI review issuance", () => {
  it("persists a partial-month snapshot, retries safely, and reuses an immutable issued review", async () => {
    ...
    try {
      ...
      expect(database.prepare(`SELECT state, period_coverage FROM coach_monthly_review_requests`).get())
        .toEqual({ state: "pending", period_coverage: "partial_month" });
      ...
      expect(second.review.output.monthlyReview).toBe("A factual monthly review.");
      ...
      expect(reviews.listIssuedMonthlyReviews(scope)).toEqual([
        expect.objectContaining({
          issuedReviewId: second.review.issuedReviewId,
          monthStartDate: "2026-08-25",
          monthEndDate: "2026-08-31",
          periodCoverage: "partial_month",
        }),
      ]);
      expect(reviews.readLatestIssuedMonthlyReviewBefore(scope, "2026-09-01"))
        .toEqual(expect.objectContaining({ issuedReviewId: second.review.issuedReviewId }));
      expect(reviews.readLatestIssuedMonthlyReviewBefore(scope, "2026-08-25")).toBeNull();
      expect(reviews.listIssuedMonthlyReviews(secondAccountScope)).toEqual([]);
      expect(() => reviews.readIssuedMonthlyReview(
        secondAccountScope,
  ...
  it("does not start a second provider call while a monthly request is pending", () => {
    const { database, scope } = setup();
    try {
      const settings = new CoachAiProviderSettingsRepository(database);
      const reviews = new CoachAiReviewRepository(database);
      const request = reviews.createOrReadMonthlyRequest(scope, input(), null, new Date(createdAtUtc));
      const first = reviews.beginMonthlyAttempt(scope, request.requestId, settings.read(), new Date(createdAtUtc));
      const second = reviews.beginMonthlyAttempt(
        scope,
        request.requestId,
        settings.read(),
        new Date("2026-08-05T12:01:00.000Z"),
