...
const sql = `DROP TRIGGER coach_ai_chat_action_drafts_scope;
DROP TRIGGER coach_ai_chat_action_drafts_no_delete;
DROP TRIGGER coach_ai_chat_action_drafts_update_guard;
DROP INDEX coach_ai_chat_action_drafts_conversation;

ALTER TABLE coach_ai_chat_action_drafts RENAME TO coach_ai_chat_action_drafts_0056_old;

CREATE TABLE coach_ai_chat_action_drafts (
  ...
  action_kind TEXT NOT NULL CHECK (action_kind IN ('reporting_currency', 'mark_notification_read', 'select_journal_account', 'notification_preferences', 'create_journal_account', 'swing_note', 'trade_style', 'trade_tags', 'rule_change', 'data_decision', 'ai_review_account_setting', 'ai_review_request')),
  preview_json TEXT NOT NULL CHECK (length(preview_json) BETWEEN 2 AND 32768 AND json_valid(preview_json) AND json_type(preview_json) = 'object'),
  private_payload_json TEXT NOT NULL CHECK (length(private_payload_json) BETWEEN 2 AND 32768 AND json_valid(private_payload_json) AND json_type(private_payload_json) = 'object'),
  ...
  canonical_command TEXT CHECK (canonical_command IS NULL OR canonical_command IN ('platform_reporting_currency_update', 'platform_notification_mark_read', 'platform_account_selection', 'platform_notification_preferences_update', 'journal_account_create', 'journal_swing_note_save', 'journal_trade_style_change', 'journal_trade_tags_replace', 'journal_trading_rules_mutate', 'journal_data_decision_resolve', 'coach_ai_review_account_setting_save', 'coach_ai_review_request_create')),
...
INSERT INTO coach_ai_chat_action_drafts SELECT * FROM coach_ai_chat_action_drafts_0056_old;
DROP TABLE coach_ai_chat_action_drafts_0056_old;
...
export const coachAiChatActionExpansionMigration: PlatformMigration = Object.freeze({
  moduleNamespace: "coach",
  migrationId: "0056_coach_ai_chat_action_expansion",
  executionOrder: 56,
