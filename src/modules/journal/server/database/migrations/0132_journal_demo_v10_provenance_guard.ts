...
// 0097 and v9 exceptions and add the approved v10 upgrade; ordinary imports stay blocked.
function permittedBatch(row: "NEW" | "batch"): string {
  ...
  AND (
    (
      ...
      ${row}.manual_idempotency_key = '0655738fd02d2b7d9be7bdf54f621a86f9db4aa8985c71ca6bd256343aeac1ac'
      AND ${row}.mapping_contract_json = '{"contractVersion":"demo_market_pack_v1","source":"mixed_synthetic_demo_analyzer_and_journal_only"}'
      AND (
        (${row}.preserved_row_count = 6 AND ${row}.statement_period_start_date = '2026-08-05' AND ${row}.statement_period_end_date = '2026-08-28')
        OR (${row}.preserved_row_count BETWEEN 193 AND 817 AND ${row}.statement_period_start_date = '2026-08-03' AND ${row}.statement_period_end_date = '2026-08-31')
      )
      AND ${row}.mapped_execution_count = ${row}.preserved_row_count
      AND EXISTS (
        SELECT 1 FROM journal_demo_pack_versions pack
        WHERE pack.demo_pack_version_id = 'd4944b87-5098-4754-a1c2-39c638c9d77c'
          AND pack.pack_key = 'daily_tracker_demo' AND pack.pack_version = 10
          AND pack.materializer_version = 'demo_canonical_journal_v10'
      )
      AND NOT EXISTS (
        SELECT 1 FROM journal_demo_lifecycle lifecycle
        WHERE lifecycle.workspace_id = ${row}.workspace_id
...
export const journalDemoV10ProvenanceGuardMigration: PlatformMigration = Object.freeze({
  moduleNamespace: "journal",
  migrationId: "0132_journal_demo_v10_provenance_guard",
  executionOrder: 132,
