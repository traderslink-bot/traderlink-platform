...
export function materializeJournalDemoAnalyzerFacts(input: Readonly<{
  ...
  for (const trade of input.source.trades) {
    ...
    const firstExecutionKey = trade.executions[0]?.packExecutionKey;
    const executionProvenance = input.executionProvenance.find(
      (fact) => fact.packExecutionKey === firstExecutionKey,
    );
    if (!executionProvenance) failure("demoAnalyzerExecutionProvenance");
...
ORDER BY round_trip.round_trip_id`).all(
      input.workspaceId,
      input.accountId,
      executionProvenance.executionVersionId,
    );
