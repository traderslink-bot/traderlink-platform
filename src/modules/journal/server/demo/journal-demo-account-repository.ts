...
export type JournalDemoPackApplicationRecord = Readonly<{
  accountId: string;
  applicationKind: "initial" | "upgrade";
  appliedAtUtc: string;
  demoPackApplicationId: string;
  demoPackVersionId: string;
  workspaceId: string;
}>;

export type JournalDemoLifecycleRecord = Readonly<{
  clearedAtUtc: string;
  clearedDemoAccountId: string;
  state: "cleared";
  userId: string;
  workspaceId: string;
}>;
...
type DemoPackApplicationRow = Readonly<{
  account_id: string; application_kind: "initial" | "upgrade"; applied_at_utc: string;
  demo_pack_application_id: string; demo_pack_version_id: string; workspace_id: string;
}>;
type DemoLifecycleRow = Readonly<{
  cleared_at_utc: string; cleared_demo_account_id: string; lifecycle_state: "cleared";
...
function mapDemoPackApplication(row: DemoPackApplicationRow): JournalDemoPackApplicationRecord {
  return Object.freeze({ accountId: row.account_id, applicationKind: row.application_kind,
    appliedAtUtc: row.applied_at_utc, demoPackApplicationId: row.demo_pack_application_id,
    demoPackVersionId: row.demo_pack_version_id, workspaceId: row.workspace_id });
}

function mapDemoLifecycle(row: DemoLifecycleRow): JournalDemoLifecycleRecord {
  return Object.freeze({ clearedAtUtc: row.cleared_at_utc,
    clearedDemoAccountId: row.cleared_demo_account_id, state: row.lifecycle_state,
...
WHERE demo.workspace_id = ? AND demo.created_for_user_id = ? AND account.status = 'active' LIMIT 1`)
  ...
  findLifecycleForUser(input: Readonly<{ userId: string; workspaceId: string }>): JournalDemoLifecycleRecord | null {
    const row = this.database.prepare<[string, string], DemoLifecycleRow>(`SELECT workspace_id, user_id,
  lifecycle_state, cleared_demo_account_id, cleared_at_utc
FROM journal_demo_lifecycle WHERE workspace_id = ? AND user_id = ? LIMIT 1`)
      .get(input.workspaceId, input.userId);
    return row ? mapDemoLifecycle(row) : null;
  }

  recordClearedLifecycle(input: Readonly<{
    clearedAtUtc: string; clearedDemoAccountId: string; userId: string; workspaceId: string;
  }>): JournalDemoLifecycleRecord {
    this.database.prepare(`INSERT OR IGNORE INTO journal_demo_lifecycle (
  workspace_id, user_id, lifecycle_state, cleared_demo_account_id, cleared_at_utc
) VALUES (?, ?, 'cleared', ?, ?)`).run(
      input.workspaceId, input.userId, input.clearedDemoAccountId, input.clearedAtUtc,
    );
    const stored = this.findLifecycleForUser(input);
    if (!stored || stored.state !== "cleared") {
      platformFailure("TRADERLINK_PLATFORM_STORAGE_VALIDATION_FAILED", { field: "demoLifecycle" });
    }
    return stored;
  }
...
WHERE execution.workspace_id = ? AND execution.current_state = 'accepted' AND demo.account_id IS NULL LIMIT 1`).get(workspaceId));
  ...
  findPackApplication(input: Readonly<{ accountId: string; demoPackVersionId: string; workspaceId: string }>): JournalDemoPackApplicationRecord | null {
    const row = this.database.prepare<[string, string, string], DemoPackApplicationRow>(`SELECT account_id, application_kind, applied_at_utc,
  demo_pack_application_id, demo_pack_version_id, workspace_id
FROM journal_demo_pack_applications
WHERE workspace_id = ? AND account_id = ? AND demo_pack_version_id = ?`)
      .get(input.workspaceId, input.accountId, input.demoPackVersionId);
    return row ? mapDemoPackApplication(row) : null;
  }

  createPackApplication(input: Readonly<{
    accountId: string;
    applicationKind: "initial" | "upgrade";
    createdAtUtc: string;
    demoPackApplicationId: string;
    ...
    this.database.prepare(`INSERT INTO journal_demo_pack_applications (
  demo_pack_application_id, workspace_id, account_id, demo_pack_version_id,
  application_kind, manifest_sha256, market_data_manifest_sha256, applied_at_utc
) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(input.demoPackApplicationId, input.workspaceId, input.accountId,
        input.manifest.demoPackVersionId, input.applicationKind,
        input.manifest.manifestSha256, input.manifest.marketDataManifestSha256,
        input.createdAtUtc);
  }

  createPackApplicationExecutionProvenance(input: Readonly<{
    accountId: string;
    createdAtUtc: string;
    demoPackApplicationExecutionProvenanceId: string;
    demoPackApplicationId: string;
    fact: JournalDemoExecutionProvenanceFact;
    workspaceId: string;
  }>): void {
    this.database.prepare(`INSERT INTO journal_demo_pack_application_execution_provenance (
  demo_pack_application_execution_provenance_id, demo_pack_application_id,
  workspace_id, account_id, execution_id, execution_version_id, analysis_policy,
  pack_execution_key, execution_fact_sha256, created_at_utc
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(input.demoPackApplicationExecutionProvenanceId, input.demoPackApplicationId,
        input.workspaceId, input.accountId, input.fact.executionId,
        input.fact.executionVersionId, input.fact.analysisPolicy,
        input.fact.packExecutionKey, input.fact.executionFactSha256,
        input.createdAtUtc);
  }
