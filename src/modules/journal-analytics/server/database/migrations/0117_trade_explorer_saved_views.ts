...
const sql = `CREATE TABLE journal_trade_explorer_saved_views (
  ...
  FOREIGN KEY (workspace_id, account_id, saved_view_id, current_version_id) REFERENCES journal_trade_explorer_saved_view_versions(workspace_id, account_id, saved_view_id, saved_view_version_id) ON UPDATE RESTRICT ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED
) STRICT;

CREATE INDEX journal_trade_explorer_saved_views_account_state
  ON journal_trade_explorer_saved_views(workspace_id, account_id, lifecycle_state, updated_at_utc DESC, saved_view_id);

CREATE TABLE journal_trade_explorer_saved_view_versions (
  ...
  view_version TEXT NOT NULL CHECK (view_version = 'trade_explorer_saved_view_v1'),
  normalized_view_json TEXT NOT NULL CHECK (
    length(normalized_view_json) BETWEEN 2 AND 65536
    AND json_valid(normalized_view_json)
    AND json_type(normalized_view_json) = 'object'
  ),
  view_sha256 TEXT NOT NULL ${sha256Check("view_sha256")},
  lifecycle_state TEXT NOT NULL CHECK (lifecycle_state IN ('active', 'retired')),
  ...
  FOREIGN KEY (workspace_id, account_id, saved_view_id) REFERENCES journal_trade_explorer_saved_views(workspace_id, account_id, saved_view_id) ON UPDATE RESTRICT ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED,
  FOREIGN KEY (authored_by_user_id) REFERENCES platform_users(user_id) ON UPDATE RESTRICT ON DELETE RESTRICT
) STRICT;

CREATE TRIGGER journal_trade_explorer_saved_views_max_active_insert
BEFORE INSERT ON journal_trade_explorer_saved_views
WHEN NEW.lifecycle_state = 'active'
BEGIN
  SELECT CASE WHEN (
    SELECT COUNT(*) FROM journal_trade_explorer_saved_views
    WHERE workspace_id = NEW.workspace_id AND account_id = NEW.account_id
      AND lifecycle_state = 'active'
  ) >= 100 THEN RAISE(ABORT, 'journal_trade_explorer_saved_view_limit') END;
END;

CREATE TRIGGER journal_trade_explorer_saved_views_no_reactivation
BEFORE UPDATE OF lifecycle_state ON journal_trade_explorer_saved_views
WHEN OLD.lifecycle_state = 'retired' AND NEW.lifecycle_state = 'active'
BEGIN
  SELECT RAISE(ABORT, 'journal_trade_explorer_saved_view_retired');
END;

CREATE TRIGGER journal_trade_explorer_saved_views_no_delete
BEFORE DELETE ON journal_trade_explorer_saved_views
BEGIN
  SELECT RAISE(ABORT, 'journal_trade_explorer_saved_view_immutable');
END;

CREATE TRIGGER journal_trade_explorer_saved_view_versions_no_update
BEFORE UPDATE ON journal_trade_explorer_saved_view_versions
BEGIN
  SELECT RAISE(ABORT, 'journal_trade_explorer_saved_view_version_immutable');
END;

CREATE TRIGGER journal_trade_explorer_saved_view_versions_no_delete
BEFORE DELETE ON journal_trade_explorer_saved_view_versions
BEGIN
  SELECT RAISE(ABORT, 'journal_trade_explorer_saved_view_version_immutable');
END`;

export const tradeExplorerSavedViewsMigration: PlatformMigration = Object.freeze({
  moduleNamespace: "journal",
  migrationId: "0117_trade_explorer_saved_views",
  executionOrder: 117,
