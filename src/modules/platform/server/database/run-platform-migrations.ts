...
export function verifyCompletedPlatformDatabase(
  ...
  const reviewedDemoPredecessor = manifestInput === undefined &&
    manifest.length === 118 &&
    manifest.at(-1)?.migrationId === "0132_journal_demo_v10_provenance_guard" &&
    manifest.at(-1)?.executionOrder === 132 &&
    manifest.at(-2)?.migrationId === "0122_journal_demo_august_provenance_guard" &&
    manifest.at(-2)?.executionOrder === 122 &&
    rows.length === 117 &&
    rows.at(-1)?.migration_id === "0122_journal_demo_august_provenance_guard" &&
    rows.at(-1)?.execution_order === 122;
