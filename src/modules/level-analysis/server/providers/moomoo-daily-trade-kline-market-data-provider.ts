...
export class MoomooDailyTradeKlineMarketDataProvider implements MarketDataProvider {
  constructor(
    ...
    private readonly options: Readonly<{ strictOwnerSession?: boolean; completeHistory?: boolean; availableThrough?: number; onDiagnostics?: (counts: Readonly<Record<string, number>>) => void }> = {},
  ) {}

  async fetch(input: MarketDataRequest): Promise<MarketDataProviderResult & { partialReasonCode?: string }> {
    const strict = Boolean(this.options.strictOwnerSession || this.options.completeHistory);
    let exhausted = false;
    ...
    const candleInput = strict ? { ...input, endTime: Math.min(input.endTime, this.options.availableThrough ?? Math.floor(Date.now() / 60000) * 60) } : input;
    ...
    const failed = (code: "coverage_unavailable" | "invalid_payload" | "provider_unavailable",
      ...
      const partial = strict ? [...candles.values()].sort((a, b) => a.time - b.time) : [];
      report();
      if (partial.length) return {
        ok: true as const, candles: partial, partialReasonCode: failureReasonCode, requestCoverage: "partial" as const,
      ...
      return { ...unavailable(code, failureReasonCode),
        requestCoverage: exhausted && failureReasonCode === "moomoo_returned_no_candles"
    ...
    const pageLimit = this.options.strictOwnerSession ? 128 : this.options.completeHistory ? 8 : MAX_PAGES;
    for (let page = 0; page < pageLimit; page += 1) {
      ...
      try {
        ...
        response = await this.request(
          ...
          { cache: "no-store", ...(strict ? { signal: AbortSignal.timeout(30_000) } : {}), headers: { Accept: "application/json", Authorization: `Bearer ${token}` } },
        );
      ...
      for (const value of payload.data.kline_list) {
        ...
        if (candle === "invalid") {
          if (strict) { diagnostics.invalid_rows_excluded += 1; continue; }
        ...
        if (prior && JSON.stringify(prior) !== JSON.stringify(candle)) {
          if (strict) { candles.delete(candle.time); conflictingTimes.add(candle.time); diagnostics.invalid_rows_excluded += 2; continue; }
      ...
      if (payload.pagination?.has_more !== true || (typeof next !== "number" && typeof next !== "string") || String(next).length === 0) {
        exhausted = payload.pagination?.has_more === false;
        if (exhausted) diagnostics.pagination_complete = 1;
        break;
      }
      cursor = String(next);
    ...
    if (strict && !exhausted) return failed("provider_unavailable", "moomoo_pagination_incomplete");
    ...
    return Object.freeze({
      ok: true,
      requestCoverage: exhausted && diagnostics.invalid_rows_excluded === 0 ? "complete" : "partial",
      candles: normalized,
