...
const sql = `CREATE TABLE level_analysis_market_session_sets (
  ...
  provider_symbol TEXT NOT NULL CHECK (
    length(provider_symbol) BETWEEN 1 AND 16 AND provider_symbol = upper(trim(provider_symbol))
    AND provider_symbol NOT GLOB '*[^A-Z0-9.-]*'
  ),
  exchange_identity TEXT NOT NULL CHECK (
    length(exchange_identity) BETWEEN 1 AND 64 AND exchange_identity = lower(trim(exchange_identity))
    AND exchange_identity NOT GLOB '*[^a-z0-9_-]*'
  ),
  trading_date_new_york TEXT NOT NULL CHECK (
    length(trading_date_new_york) = 10 AND trading_date_new_york GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]'
  ),
...
CREATE INDEX level_analysis_market_session_sets_work
  ON level_analysis_market_session_sets(current_status, lease_expires_at_utc, updated_at_utc);

CREATE TRIGGER level_analysis_market_session_sets_no_delete
BEFORE DELETE ON level_analysis_market_session_sets BEGIN
  SELECT RAISE(ABORT, 'level_analysis_market_session_history_required');
END;
...
CREATE TABLE level_analysis_daily_trade_jobs (
  ...
  market_session_set_id TEXT NOT NULL ${uuidCheck("market_session_set_id")},
  desired_coverage_end_utc TEXT NOT NULL ${utcCheck("desired_coverage_end_utc")},
  next_attempt_at_utc TEXT NOT NULL ${utcCheck("next_attempt_at_utc")},
  ...
  updated_at_utc TEXT NOT NULL ${utcCheck("updated_at_utc")},
  CHECK (updated_at_utc >= created_at_utc),
  UNIQUE (workspace_id, account_id, round_trip_version_id, desired_coverage_end_utc),
...
CREATE INDEX level_analysis_daily_trade_jobs_available
  ON level_analysis_daily_trade_jobs(status, next_attempt_at_utc, lease_expires_at_utc);

CREATE TRIGGER level_analysis_daily_trade_jobs_no_delete
BEFORE DELETE ON level_analysis_daily_trade_jobs BEGIN
  SELECT RAISE(ABORT, 'level_analysis_daily_trade_job_history_required');
END;
...
CREATE TABLE journal_round_trip_daily_trade_analysis_versions (
  ...
  market_session_set_version_id TEXT ${uuidCheck("market_session_set_version_id")},
  status TEXT NOT NULL CHECK (status IN ('pending', 'ready', 'no_coverage', 'provider_unavailable', 'expired')),
  analyzer_contract_version TEXT NOT NULL CHECK (analyzer_contract_version = 'daily_trade_analyzer_v1'),
  created_at_utc TEXT NOT NULL ${utcCheck("created_at_utc")},
...
CREATE TRIGGER journal_round_trip_daily_trade_analyses_no_delete
BEFORE DELETE ON journal_round_trip_daily_trade_analyses BEGIN
  SELECT RAISE(ABORT, 'journal_daily_trade_analysis_history_required');
END;

CREATE TRIGGER journal_round_trip_daily_trade_analysis_versions_no_update
BEFORE UPDATE ON journal_round_trip_daily_trade_analysis_versions BEGIN
  SELECT RAISE(ABORT, 'journal_daily_trade_analysis_version_immutable');
END;

CREATE TRIGGER journal_round_trip_daily_trade_analysis_versions_no_delete
BEFORE DELETE ON journal_round_trip_daily_trade_analysis_versions BEGIN
  SELECT RAISE(ABORT, 'journal_daily_trade_analysis_history_required');
END;`;

export const dailyTradeYahooAnalyzerMigration: PlatformMigration = Object.freeze({
  moduleNamespace: "level_analysis",
  migrationId: "0023_daily_trade_yahoo_analyzer",
  executionOrder: 23,
