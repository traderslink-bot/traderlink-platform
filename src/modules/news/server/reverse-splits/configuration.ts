...
export type ReverseSplitConfiguration = Readonly<{
  eodhdToken: string; secUserAgent: string; dataUseApproved: boolean; privatePreview: true;
  discord: Readonly<{ channelId: string; guildId: string; botToken: string }> | null;
}>;
export function reverseSplitPrivatePreviewEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.REVERSE_SPLIT_ENABLED === "true" && env.REVERSE_SPLIT_PRIVATE_PREVIEW_ENABLED === "true";
}
export function reverseSplitConfiguration(env: NodeJS.ProcessEnv = process.env): ReverseSplitConfiguration | null {
  if (!reverseSplitPrivatePreviewEnabled(env)) return null;
  ...
  if (env.DISCORD_REVERSE_SPLIT_DELIVERY_ENABLED === "true") {
    if (env.DISCORD_REVERSE_SPLIT_PRIVATE_CHANNEL_CONFIRMED !== "true") throw new Error("reverse_split_private_channel_not_confirmed");
  ...
  return { eodhdToken, secUserAgent, dataUseApproved, privatePreview: true, discord };
}
