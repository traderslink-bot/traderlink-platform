...
const includeNasdaq0750T1Halts = process.env.TRADERLINK_INCLUDE_NASDAQ_0750_T1_HALTS === "true";
...
function canonicalEasternDate(value: string | null): string | null {
...
export function parseNasdaqTradeHalts(xml: string): readonly MarketHalt[] { return Object.freeze((xml.match(/<item(?:\s[^>]*)?>[\s\S]*?<\/item>/giu) ?? []).flatMap((item) => { const ticker = xmlTag(item, "ndaq:IssueSymbol"); const haltDateEt = canonicalEasternDate(xmlTag(item, "ndaq:HaltDate")); const haltTimeEt = xmlTag(item, "ndaq:HaltTime"); const reasonCode = xmlTag(item, "ndaq:ReasonCode"); const description = reasonCode ? nasdaqReasons[reasonCode] : null; if (!validTicker(ticker) || !haltDateEt || !haltTimeEt || !reasonCode || !description || (!includeNasdaq0750T1Halts && reasonCode === "T1" && /^07:50:00(?:\.\d+)?$/u.test(haltTimeEt))) return []; return [Object.freeze({ haltDateEt, haltTimeEt, issueName: xmlTag(item, "ndaq:IssueName") ?? ticker, market: xmlTag(item, "ndaq:Market") ?? "NASDAQ", reasonCode, reasonDescription: description, resumptionQuoteTimeEt: xmlTag(item, "ndaq:ResumptionQuoteTime"), resumptionTradeTimeEt: xmlTag(item, "ndaq:ResumptionTradeTime"), source: "nasdaq" as const, ticker })]; })); }
...
function splitEasternDateTime(value: string): Readonly<{ date: string; time: string }> | null { const match = value.match(/(\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2})\s+(\d{1,2}:\d{2}(?::\d{2})?)/u); if (!match) return null; const date = canonicalEasternDate(match[1]); return date ? Object.freeze({ date, time: match[2].padStart(8, "0") }) : null; }
export function parseNyseTradeHalts(csv: string): readonly MarketHalt[] { const rows = csvRows(csv); const headers = rows[0]?.map((header) => header.toLowerCase().replace(/[^a-z]/gu, "")) ?? []; const field = (row: readonly string[], ...names: string[]) => { const index = headers.findIndex((header) => names.includes(header)); return index >= 0 ? row[index] ?? "" : ""; }; return Object.freeze(rows.slice(1).flatMap((row) => { const reason = field(row, "reason", "haltreason"); const mapped = nyseReasonMap.find(([pattern]) => pattern.test(reason)); const ticker = field(row, "symbol", "issuesymbol").toUpperCase(); const market = field(row, "market", "exchange") || "NYSE"; const combined = splitEasternDateTime(field(row, "halt", "haltdatetime", "haltdateandtime")); const haltDateEt = canonicalEasternDate(field(row, "haltdate", "date")) ?? combined?.date ?? ""; const haltTimeEt = field(row, "halttime", "time") || combined?.time || ""; if (!/^NYSE(?:\s|$)/iu.test(market) || !mapped || !validTicker(ticker) || !haltDateEt || !haltTimeEt) return []; return [Object.freeze({ haltDateEt, haltTimeEt, issueName: field(row, "company", "issuename", "name") || ticker, market, reasonCode: mapped[1], reasonDescription: mapped[2], resumptionQuoteTimeEt: field(row, "quotetime", "resumptionquotetime") || null, resumptionTradeTimeEt: field(row, "resumetime", "resumptiontradetime") || null, source: "nyse" as const, ticker })]; })); }
