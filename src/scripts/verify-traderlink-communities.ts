...
try{
  ...
  expectDenied(()=>platform.sendCoachingMessage({communityId:first.communityId,actor:{userId:OWNER,displayName:"Owner",discordRoleIds:[]},relationshipId:relationship,body:"Blocked until enabled.",atUtc:NOW}),"An active student must not receive messaging access until the coach enables it.");
  expectDenied(()=>platform.requestTradeReview({communityId:first.communityId,actor:{userId:OWNER,displayName:"Owner",discordRoleIds:[]},relationshipId:relationship,title:"Blocked review",studentContext:"",atUtc:NOW}),"An active student must not receive trade-review access until the coach enables it.");
  ...
  assert(coachingSnapshot.coachingMessages.length===1&&coachingSnapshot.tradeReviews[0]?.status==="completed"&&coachingSnapshot.coachingTasks[0]?.status==="completed"&&coachingSnapshot.coachingRecords.length===1,"Coach and student work must stay inside their active relationship.");
