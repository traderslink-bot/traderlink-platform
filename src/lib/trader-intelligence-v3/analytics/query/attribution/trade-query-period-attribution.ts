...
export function executeTradeQueryPeriodAttribution(request: TradeQueryPeriodAttributionRequest): { readonly ok: true; readonly value: TradeQueryPeriodAttributionResult } | { readonly ok: false; readonly error: AnalyticalContractFailure } {
  ...
  for (const identity of identities) {
    ...
    segments.push(Object.freeze({
      ...
      relativeChange: ratioOrUnavailable("relative_change", BigInt(decimalRatio(base.net).numerator) === BigInt("0") ? null : divide(subtract(decimalRatio(target.net), decimalRatio(base.net)), { numerator: (BigInt(decimalRatio(base.net).numerator) < BigInt("0") ? -BigInt(decimalRatio(base.net).numerator) : BigInt(decimalRatio(base.net).numerator)).toString(), denominator: decimalRatio(base.net).denominator })),
