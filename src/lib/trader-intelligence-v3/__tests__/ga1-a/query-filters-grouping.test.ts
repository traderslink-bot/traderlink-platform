...
describe("GA1-A independent filters and deterministic grouping", () => {
  it.each([
    ...
    ["entry session", [{ kind: "entry_session", values: ["premarket"] }]],
    ["exit session", [{ kind: "exit_session", values: ["after_hours"] }]],
    ["entry-to-exit session", [{ kind: "session_transition", values: ["premarket_to_regular"] }]],
