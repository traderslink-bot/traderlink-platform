...
export function scaleTradeIndicatorResult(result: TradeExecutionIndicatorResult, multiplier: string): TradeExecutionIndicatorResult {
  ...
  const during = (value: TradeExecutionIndicatorResult["duringTrade"]["oneMinute"]) => value ? {
    ...
    episodes: value.episodes.map((episode) => ({ ...episode, price: scale(episode.price)!, reclaimPrice: scale(episode.reclaimPrice),
      ...(episode.lossContext ? { lossContext: { ...episode.lossContext, context: context(episode.lossContext.context) } } : {}),
      ...(episode.reclaimStudy ? { reclaimStudy: { ...episode.reclaimStudy, price: scale(episode.reclaimStudy.price)!,
        context: context(episode.reclaimStudy.context),
        horizons: episode.reclaimStudy.horizons.map((horizon) => ({ ...horizon, changePerShare: scale(horizon.changePerShare) })),
        untilClosure: { ...episode.reclaimStudy.untilClosure, changePerShare: scale(episode.reclaimStudy.untilClosure.changePerShare)! },
      } } : {}),
