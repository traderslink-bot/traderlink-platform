...
| Candle adapter and deterministic analyzer module | In progress | Protected Yahoo simulation endpoint now returns volume-aware, evidence-gated results; the app shows the selected symbol's live exit finding or no-feedback reason. |
| Execution-context candle patterns | In progress | Approved five-family vocabulary: expansion, engulfing, rejection wick, compression/break, and high-volume exhaustion. Definitions and execution-zone eligibility are recorded in the plan before detector implementation. |
