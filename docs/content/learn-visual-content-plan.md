...
```yaml
featured_image: "/academy/images/chart-reading/support-resistance-levels-diagram.png"
...
Historical first-batch recommendation from the original visual plan:
...
The first Academy-wide visual/UI-readiness review is complete:

```text
docs/content/learn-academy-visual-ui-readiness-review.md
```

That review supersedes the original first-batch list for current Academy work.
...
The review found that the Academy is ready for UI planning from a content metadata perspective. The remaining content-only work is targeted SVG support, not another broad lesson rewrite.

Current highest-priority visual batch:
...
1. `indicator-lag-price-vs-signal.svg`
2. `indicator-overload-duplicate-inputs.svg`
3. `moving-average-trend-and-chop.svg`
4. `vwap-hold-loss-reclaim-review.svg`
5. `rsi-macd-momentum-context.svg`
6. `bollinger-atr-volatility-context.svg`

Use the readiness review as the source of truth for the full visual backlog across Technical Indicators, Risk, Small-Cap/Dilution, Trade Review, Psychology, Trading Styles, Trading Foundations, and deeper News/SEC filing support.
