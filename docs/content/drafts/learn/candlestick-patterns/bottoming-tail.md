---
title: "Bottoming Tail Candle"
slug: "/learn/candlestick-patterns/bottoming-tail/"
primary_keyword: "bottoming tail candle"
secondary_keywords:
  - "bottoming tail candlestick"
  - "lower wick candle"
  - "buyer rejection candle"
  - "candlestick reversal context"
search_intent: "Learn bottoming tail candles as lower-wick recovery context near support, not guaranteed reversals."
status: "draft"
product_area: "Education"
availability: "educational"
content_type: "academy_lesson"
funnel_stage: "awareness"
priority: "3"
cta: "Trader Intelligence is being built to help traders review whether candle patterns supported a completed decision or became a reason to react without context."
recommended_previous: "/learn/candlestick-patterns/pin-bar/"
recommended_next: "/learn/candlestick-patterns/topping-tail/"
academy_level: "Practical"
academy_order: 9
academy_module: "Tail Candles"
academy_course: "Candlestick Patterns In Context"
learning_track: "Candlestick Patterns In Context"
internal_links:
  - "/learn/candlestick-patterns/hammer/"
  - "/learn/candlestick-patterns/long-wick-candle/"
  - "/learn/support-levels/"
  - "/learn/price-rejection/"
  - "/learn/volume/"
visual_assets:
  - "/images/learn/chart-reading/hammer-support-context.svg"
schema:
  - "FAQPage"
last_reviewed: "2026-05-17"
meta_title: "Bottoming Tail Candle Explained"
meta_description: "Learn what bottoming tails show and how to review lower-wick recovery with support, volume, and risk."
---
# Bottoming Tail Candle

This lesson is part of the **Candlestick Patterns In Context** course. It follows [Pin Bar](/learn/candlestick-patterns/pin-bar/) and leads into [Topping Tail Candle](/learn/candlestick-patterns/topping-tail/).

A bottoming tail candle has a long lower wick that shows price dropped during the candle and recovered before the close. It can appear near support, after a flush, or during a failed breakdown attempt.

The candle can be useful, but only if the trader reviews whether the lower area actually mattered afterward.

![Candlestick chart showing a hammer-style lower wick near support with volume and follow-through context.](/images/learn/chart-reading/hammer-support-context.svg)

## Lesson Objective

By the end of this lesson, you should be able to:

- Recognize the pattern without treating the name as a trade plan.
- Read the candle beside levels, volume, trend, and liquidity.
- Identify common failure conditions.
- Review entry, exit, and risk behavior after the trade.
- Use the pattern as context rather than prediction.

## Quick Definition

A bottoming tail is a candle with a long lower wick and a close above the low.

- Price tested lower during the candle.
- The candle closed away from the low.
- The lower wick may show rejection below.
- It often overlaps with hammer-style candles.
- The wick low becomes an important review point.

A bottoming tail shows lower-price rejection during the candle, not proof that a bottom is in.

## What The Candle Shows

Bottoming tails show recovery from lower prices.

- Potential demand near support.
- A possible failed breakdown.
- A possible temporary bounce after selling pressure.
- A possible liquidity flush.
- A need to watch whether the wick low holds.

## Context That Matters

Bottoming tails need support and follow-through context.

- Support or low-of-day area.
- Volume during the wick.
- Whether price reclaims a level.
- Whether the wick low holds.
- Spread and liquidity.
- Broader trend and catalyst context.

## When It Can Mislead

Bottoming tails can mislead during downtrends because price can print several lower wicks and continue lower.

## Realistic Example

A stock flushes below the prior low, recovers, and closes back above the level with a long lower wick. The review asks whether volume increased, whether the level held afterward, and whether the trader chased far from risk.

## Common Mistakes

One common mistake is assuming the bottom is in.

Another mistake is ignoring the broader downtrend.

Traders also make mistakes when they chase far above the wick low.

Another mistake is ignoring volume and liquidity.

A final mistake is holding after the wick low fails.

## Practical Checklist

- The candle is identified correctly.
- The pattern formed near a meaningful area, not random noise.
- Volume and liquidity are reviewed.
- The broader trend and nearby levels are checked.
- Risk is defined without relying on the candle name.
- The next candles are reviewed for follow-through or failure.
- The trade can be reviewed honestly after completion.

## Apply This In Review

- What candle pattern did I think I saw?
- Where did it form on the chart?
- What level, trend, or volume context mattered?
- Did I react to the name or follow a plan?
- Did the pattern follow through or fail?
- Where was invalidation?
- What should I tag for future review?

Bottoming tail review should focus on lower-wick quality, level location, and failure behavior.

## Trader Intelligence Bridge

Trader Intelligence is being built to help traders review completed trades by candle pattern, location, volume, entry timing, failure behavior, and repeated reaction patterns.

## Related Lessons

- [Hammer Candlestick](/learn/candlestick-patterns/hammer/)
- [Long Wick Candle](/learn/candlestick-patterns/long-wick-candle/)
- [Support Levels](/learn/support-levels/)
- [Price Rejection](/learn/price-rejection/)
- [Volume](/learn/volume/)

## FAQ

### What is a bottoming tail candle?

It is a candle with a long lower wick and a close above the low.

### Is a bottoming tail bullish?

It can be constructive near support, but it is not automatically bullish.

### Does it guarantee reversal?

No. Price can break below the wick low.

### Is it the same as a hammer?

They are related; a hammer is one form of lower-wick rejection.

### Why does volume matter?

Volume helps review whether the rejection had meaningful participation.

### How should it be reviewed?

Review level, volume, wick low, reclaim behavior, entry timing, and failure.

## Educational Disclaimer

This guide is for educational purposes only and is not financial advice. Candlestick patterns can support chart review, but they do not predict price, guarantee trading results, or replace risk management.