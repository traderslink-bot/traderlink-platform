...
- For a ticker and Eastern trading date, only the first recorded halt can notify. It sends the initial alert once, then only factual expected quote/trade-time updates when those official times first appear or change. Newly discovered or changed times are queued before checking for silent closure, even when first detected at or after their scheduled time. If the posted expected trade time arrives without an official revision, the sequence ends silently. Already-ended sequences are not reopened or replayed. Later same-day halts stay recorded but do not notify, and alerts never claim a confirmed unhalt or resumption. See the [WZRD correction record](halt-alert-discord-channel-progress.md#wzrd-resumption-update-correction-2026-09-24).
