...
- De-duplicate a halt across sources and notify once per halt/device. Every delivery of the same stored halt uses the same browser notification tag, so a retry or duplicate active subscription cannot create a second visible notification on that device; a factual later halt retains its distinct stored halt and remains visible.
