...
- Workspace now places the existing trade filters in one desktop row directly above the table and provides one responsive Display action for saved Workspace card visibility controls; its drawer also contains the trade filters on mobile. Top Tickers now has its own account-and-user-scoped display preference.
- Workspace Display keeps the full saved Current Focus available while its card is hidden, updates card visibility without a full-page refresh, isolates each switch's saving state so one change does not dim every control, and shares the latest saved revisions with the Focuses, Rules, and PR Scanner controls.
- Workspace trade filters now prioritize the direct table-data refresh before the delayed URL/card synchronization so the table does not compete with an immediate full Workspace navigation.
