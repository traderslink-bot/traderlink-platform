...
**Save Note** sits immediately below the note editor, beside the reminder
**Save multiple note types to a session**. The bottom-of-review **Save Review**
action saves the selected session tags and changed custom-rule results; it does
not silently save or overwrite text that is still in the note editor.
