...
| Communities | Discord-server onboarding, global Communities Administration, server-owner management, custom community roles and permissions, private member pages, server alerts and watchlists, permission-derived staff workspaces, community-scoped human coaching, optional future platform fee configuration, named member activity, Discord channel delivery and Tier 2 partner attribution | Peer module at `/admin/communities` plus plural `/communities` server surfaces, preserving singular `/community` for personal Community Watchlists and a later TraderLink-native social layer | [TraderLink Communities Partner Platform Plan](traderlink-communities-partner-platform-plan.md) records the approved partner-distribution direction and complete target inventory. Communities 1-7 are assembled with focused cross-community, role, coaching, Journal-grant, activity and Tier 2 proof passing. Final integrated visual approval is intentionally deferred to staging. It preserves the official Premium Watchlist, personal Community Watchlists, stable Platform identity, Journal ownership and existing Affiliate first-touch boundary. |
