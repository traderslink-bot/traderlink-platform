...
**Focused analytics pages:** The owner-approved [Analytics Page Architecture Plan](analytics-pages-architecture-plan.md) separates Overview, Performance, Results, Execution and Timing so they do not become duplicate metric pages. Overview implementation is active under its [progress record](analytics-overview-progress.md); it will contain only the approved useful account snapshot cards and one monthly P/L chart.
