...
| Preset/custom rule activation or settings | Not implemented yet in Chat | Yes | Existing `/api/intelligence/rules` command; Chat-specific persisted expiring draft/confirm contract still required. Trader-confirmed activation is allowed; autonomous activation is forbidden. |
| Trade tags | Not implemented yet in Chat | Yes | Existing Daily/Swing tag commands; Chat-specific persisted expiring draft/confirm contract still required |
