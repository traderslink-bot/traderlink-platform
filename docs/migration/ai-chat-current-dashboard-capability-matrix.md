...
| AI Reviews | `/ai-reviews`, saved weekly/two-week/monthly review detail routes | Saved-review service, `CoachAiReviewAvailabilityService`, request service and review schedule/settings commands | List/read weekly, two-week and monthly saved reviews, compare a review with later exact facts, request an eligible review when the canonical command allows it, and confirm delivery settings. | Saved read, delivery schedule, on/off and eligible request confirmation implemented |
...
| Swing note | Implemented | Yes | Persisted expiring Chat action draft re-reads the exact dated note and revision, then confirms through `JournalSwingNoteService.save` with Chat-specific idempotency |
