...
- [x] Port bounded activation/materializer and server-side read-only/Analyzer
      guards onto current ancestry. Resolver activation remains unavailable
      until the immutable financial pack validates; it never falls back to
      placeholder prices, executions, or Analyzer facts.
- [x] Store the exact eight sanitized normalized candle arrays in a checksum-
      gated immutable in-repository pack. The resolver now accepts the pack
      only after both canonical Journal and ordinary immutable Analyzer facts
      can be materialized in the same transaction, without a provider call or
      Analyzer job.
- [x] Add the approved Workspace and data-page demo indicators in source: the
      Workspace callout uses the approved red border/copy and Daily Tracker
      views remain readable but hide manual entry and render read-only. The
