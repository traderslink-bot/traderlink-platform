...
test.describe("app feature regression", () => {
  ...
  test("shows the analytics product intelligence surfaces", async ({
    ...
    await visitAndAssertRoute(page, "/analytics", "Analytics");
  ...
  test("shows the coach product loop with calibrated coaching surfaces", async ({
    ...
    await visitAndAssertRoute(page, "/coach", /^Review/);
    for (const heading of [
      "Coach Review Queue",
      "Evidence Cards",
      "Session Timing",
    ...
    await expect(
      page.getByText("Advanced coach details", { exact: true }),
    ).toBeVisible();
