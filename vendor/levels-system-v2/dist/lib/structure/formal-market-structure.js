...
function sortedUsableCandles(candles, timeframe, asOfTimestamp) {
...
export function buildFormalMarketStructureContext(request) {
    ...
    const asOfTimestamp = request.asOfTimestamp === undefined ? undefined : parseTimestamp(request.asOfTimestamp);
    const { candles, diagnostics: filterDiagnostics } = sortedUsableCandles(request.candles, timeframe, asOfTimestamp);
