...
await writeFile(join(outDir, "nasdaq-deep-daily-4h-backfill-plan.md"), [
...
console.log(`[NasdaqDeepDaily4hBackfill] mode=${plan.mode} symbols=${plan.totals.symbols} selected=${plan.totals.selectedForFetch} requests=${plan.totals.selectedRequests} under500=${plan.totals.selectedUnder500} over500=${plan.totals.selectedOver500}`);
    ...
    for (const task of selectedTasks) {
        for (const timeframe of task.fetchTimeframes) {
            ...
            try {
                ...
                console.log(`[NasdaqDeepDaily4hBackfill] ${task.segment} ${task.symbol} ${timeframe}: fetched ${response.candles.length}`);
            }
            catch (error) {
