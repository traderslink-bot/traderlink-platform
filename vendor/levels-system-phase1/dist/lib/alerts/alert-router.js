...
function buildSnapshotTradeMapLines(payload) {
    ...
    if (support && resistance && supportLabel && resistanceLabel) {
        ...
        if (bandWidth <= 0.12) {
            lines.push(`Current structure: ${payload.symbol} is range-bound between ${supportLabel} and ${resistanceLabel}.`);
            ...
            lines.push(`Current structure: ${payload.symbol} is trading between ${supportLabel} and ${resistanceLabel}.`);
    ...
    if (resistance && resistanceLabelWithDistance) {
        if (nextResistanceLabelWithDistance) {
            lines.push(`Cleaner above: acceptance above ${resistanceLabelWithDistance} would shift attention toward ${nextResistanceLabelWithDistance}.`);
        }
        else {
            lines.push(`Cleaner above: acceptance above ${resistanceLabelWithDistance} would be constructive; higher resistance needs a fresh level check before treating the path as open.`);
        }
    }
    if (support && supportLabelWithDistance) {
        ...
        if (deeperSupportLabelWithDistance) {
            lines.push(`Broader support: a clean loss of ${supportLabel} as a whole area would shift attention toward ${deeperSupportLabelWithDistance}.`);
        }
    }
    if (intradaySupport && (!support || formatSnapshotEntryPrice(intradaySupport) !== formatSnapshotEntryPrice(support))) {
        lines.push(`Short-term momentum support: ${formatSnapshotEntryLabelWithDistance(intradaySupport, payload.currentPrice, "support")}.`);
    }
