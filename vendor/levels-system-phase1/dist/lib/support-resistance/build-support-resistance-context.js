...
export async function buildSupportResistanceContextFromNormalizedCandles(params) {
    const provider = new InMemoryHistoricalCandleProvider(params.symbol, params.candlesByTimeframe, params.providerByTimeframe);
...
export function normalizeSharedSupportResistanceCandles(candles, asOfTimestamp) {
    return sortSharedCandles((candles ?? [])
        .map(normalizeCandle)
