...
function closedCandlesThrough(candles, timestamp, timeframe) {
    ...
    return candles
        .filter((candle) => candle.timestamp + intervalMs <= timestamp)
...
function higherTimeframeAsOfCutoffs(timestamp) {
    const sessionDate = sessionDateForTimestamp(timestamp);
    return {
        daily: Math.max(0, nySessionTimestamp(sessionDate, 0, 0) - ONE_MINUTE_MS),
        "4h": Math.max(0, timestamp - FOUR_HOUR_MS),
        "5m": timestamp,
    };
}
...
function diagnosticsForTradeWindow(params) {
    const diagnostics = [
        ...
        },
        {
            ...
            severity: "info",
            message: `Daily context ended before the trade session (${iso(params.supportResistanceCutoffs.daily)}); ` +
                `4h context used only completed candles through ${iso(params.supportResistanceCutoffs["4h"])}.`,
        },
    ];
...
function buildExecutionRelationFacts(params) {
    return (params.executions ?? []).map((execution) => {
        ...
        const executionCandles = params.tradeWindow.allCandles.filter((candle) => candle.timestamp <= timestamp);
...
export async function buildTradeAnalysisCandleContext(request) {
    ...
    const indicatorBasisCandles = tradeWindowResponse.candles
        .filter((candle) => candle.timestamp >= indicatorBasisStartTimestamp && candle.timestamp <= requestedEndTimestamp)
        .sort((left, right) => left.timestamp - right.timestamp);
    ...
    const tradeWindow = {
        timeframe: tradeWindowResponse.timeframe,
    ...
    const diagnostics = diagnosticsForTradeWindow({
        ...
        priceAnchor,
        precomputedDiagnostics: tradeWindowFetch.diagnostics,
    });
